...
**ひとつひとつの実行ステップを span ツリーで直感的に可視化**します。
...
| Profile | 用途 |
| --- | --- |
| `raw-local-receiver` | Docker 不要。このリポジトリの Local Ingestion Monitor へ直接送信（おすすめ） |
...
Docker も Langfuse も Copilot 実実行も不要です。保存済みの raw OTLP JSON や合成データを使って、データ変換および静的ダッシュボード生成パイプラインの動作のみを試せます。
