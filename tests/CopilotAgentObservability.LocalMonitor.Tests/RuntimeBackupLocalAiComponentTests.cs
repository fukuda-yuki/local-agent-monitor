...
public sealed class RuntimeBackupLocalAiComponentTests
{
    ...
    public async Task BackupAndRestore_RejectExpiredSessionSnapshotWithForgedAnchor()
    {
        ...
        try
        {
            ...
            var restore=service.Restore(corrupt,Path.Combine(root,"restored.db"),new RuntimeRestoreOptions());Assert.False(restore.Success);Assert.Equal(RuntimeBackupErrorCodes.ArchiveInvalid,restore.ErrorCode);
        }
        finally{SqliteConnection.ClearAllPools();Directory.Delete(root,true);}
    }
    [Theory]
    ...
    private static void Write(ZipArchive archive,string name,byte[] bytes){var entry=archive.CreateEntry(name,CompressionLevel.NoCompression);using var output=entry.Open();output.Write(bytes);}
