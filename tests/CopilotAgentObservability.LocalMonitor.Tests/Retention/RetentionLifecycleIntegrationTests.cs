...
public sealed class RetentionLifecycleIntegrationTests
{
    ...
    [Fact]
    public async Task AdmittedOperationGrant_RemainsUsableAcrossCleanupUntilExactLeaseExpiry()
    {
        var path = CopyRawFixture();
        try
        {
            ...
            store.CreateSchema();
            var rawRecordId = RawScalar(path, "SELECT id FROM raw_records ORDER BY id LIMIT 1;");
            ...
            var t1 = item.ExpiresAt;
            RawExecute(path, "UPDATE retention_items SET expires_at='2027-06-01T00:00:00.0000000+00:00' WHERE item_id <> $id;", ("$id", item.ItemId));
            time.Advance(t1 - time.GetUtcNow() - TimeSpan.FromTicks(1));
            store.RegisterAdapterCoverage(new RetentionAdapterRegistry([
                new UnreachableAdapter(RetentionStoreKind.SessionEventContent),
            ...
            var leaseExpiry = grant.LeaseExpiresAt;
            try
            {
                time.Advance(TimeSpan.FromTicks(1));
                var batch = await store.PrepareCleanupBatchAsync(t1, RetentionV1Constants.ExpiryScanItemLimit, RetentionV1Constants.ClaimBatchLimit, TimeSpan.FromSeconds(30), CancellationToken.None);
                var afterCleanup = Assert.IsType<RetentionCatalogItem>(store.Find(key));
                Assert.Equal(RetentionItemLifecycle.DeletionQueued, afterCleanup.State);
                Assert.Equal(item.Revision + 2, afterCleanup.Revision);
                Assert.Equal(t1, afterCleanup.ReadDeniedAt);
                Assert.Equal(t1.ToString("O"), RawScalarText(path, $"SELECT queued_at FROM retention_items WHERE item_id='{afterCleanup.ItemId}';"));
                Assert.Equal(afterCleanup.ItemId, Assert.Single(batch.Work).ItemId);

                Assert.True(ConsumeOperationGrant(path, grant, rawRecordId, leaseExpiry.AddTicks(-1)));

                var quiescing = await store.TryClaimDeletionAsync(
                    new RetentionWorkReference(afterCleanup.ItemId, afterCleanup.Revision, RetentionWorkKind.Queued),
                    "timeline-deletion-owner",
                    leaseExpiry.AddTicks(-1),
                    CancellationToken.None);
                Assert.Equal(RetentionClaimDisposition.Quiescing, quiescing.Disposition);
                Assert.Equal(leaseExpiry, quiescing.QuiescenceRetryAt);

                Assert.False(ConsumeOperationGrant(path, grant, rawRecordId, leaseExpiry));

                var claimed = await store.TryClaimDeletionAsync(
                    new RetentionWorkReference(afterCleanup.ItemId, afterCleanup.Revision, RetentionWorkKind.Queued),
                    "timeline-deletion-owner",
                    leaseExpiry,
                    CancellationToken.None);
                Assert.Equal(RetentionClaimDisposition.Claimed, claimed.Disposition);
                Assert.Equal(item.Revision + 3, Assert.IsType<RetentionDeletionClaim>(claimed.Claim).Fence.ExpectedRevision);
                var deleting = Assert.IsType<RetentionCatalogItem>(store.Find(key));
                Assert.Equal(RetentionItemLifecycle.Deleting, deleting.State);
                Assert.Equal(item.Revision + 3, deleting.Revision);
            }
            finally { await lease.DisposeAsync(); }
        }
        finally { DeleteRaw(path); }
    }
    ...
    private static string CopyRawFixture()
    {
        var source = Path.Combine(AppContext.BaseDirectory, "TestData", "SchemaMigrations", "monitor", "monitor-v5.sqlite");
        var target = Path.Combine(Path.GetTempPath(), $"retention-admitted-grant-{Guid.NewGuid():N}.sqlite");
        File.Copy(source, target);
        return target;
    }

    private static long RawScalar(string path, string sql)
    {
    ...
    private static string RawScalarText(string path, string sql)
    {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={path};Pooling=False");
        connection.Open();
        using var command = connection.CreateCommand();
    ...
    private static void RawExecute(string path, string sql, params (string Name, object Value)[] parameters)
    {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={path};Pooling=False");
        connection.Open();
        using var command = connection.CreateCommand();
    ...
    private static void DeleteRaw(string path)
    {
