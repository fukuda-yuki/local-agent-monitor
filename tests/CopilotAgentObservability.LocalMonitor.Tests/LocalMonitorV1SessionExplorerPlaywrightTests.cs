...
public sealed class LocalMonitorV1SessionExplorerPlaywrightTests
{
    ...
    public async Task ObservationTimeLabelsOtelOnlySessionWithoutInventingLifecycle()
    {
        ...
        await Expect(page.Locator("[data-session-started]").First).ToContainTextAsync("最終記録");
        await Expect(page.Locator("[data-session-status]").First).ToHaveTextAsync("状態不明");
        await Expect(page.Locator("[data-session-started]").Nth(1)).Not.ToContainTextAsync("最終記録");
    }
    ...
    [InlineData("partial", "ライフサイクルまたは入力の一部欠落", false)]
    [InlineData("rich", "内容または終了状態の一部欠落", false)]
    ...
    public async Task RepositoryAiPreviewUsesSharedFactStatesWithoutFlatteningRecordedValuesOrHostileText()
    {
        ...
        await Expect(content.Locator("[data-fact-state='unsupported'] p")).ToHaveTextAsync(new[]
        {
            ...
            "取得元: vscode。取得元がこの項目に未対応です。",
        });
        await Expect(content.Locator("[data-fact-state='capture-gap'] p")).ToContainTextAsync("記録に欠落があります");
        await Expect(content.Locator("[data-fact-state='raw-not-captured'] p").First).ToContainTextAsync("取得時に保存されていません");
        ...
        await Expect(content.GetByText("取得元: vscode。取得元がこの項目に未対応です。", new() { Exact = true })).ToBeVisibleAsync();
    ...
    public async Task RepositoryAiRendersClosedResultAndOnlyCanonicalEvidenceAsAnExactLink()
    {
        ...
        await page.Locator("#session-ai-open").ClickAsync(); await page.Locator("#session-ai-preview").ClickAsync();
        var previewContent = page.Locator("#session-ai-preview-content");
        await Expect(previewContent.GetByRole(AriaRole.Heading, new() { Name = "対象の技術情報 1件", Exact = true })).ToBeVisibleAsync();
        ...
        await Expect(previewContent).ToContainTextAsync("なし");
        await Expect(previewContent).ToContainTextAsync("記録形式を読み取れません");
        await Expect(previewContent).ToContainTextAsync("表示上限を超えています");
        await Expect(previewContent).ToContainTextAsync("値が一致しません");
        await Expect(previewContent).ToContainTextAsync("必要な記録がそろっています");
        await Expect(previewContent).ToContainTextAsync("ライフサイクルまたは入力の一部欠落");
        ...
        await Expect(renderedResult).ToContainTextAsync("AIによる解釈");
        await Expect(renderedResult).ToContainTextAsync("分析対象の技術情報");
        await Expect(renderedResult).ToContainTextAsync("記録時点の技術情報");
    ...
    [Fact]
    [Trait("ValidationLane", "Nightly")]
    public async Task RepositoryAiAcceptsExactOneMiBResultInsideBoundedEnvelopeWithEscapedMarker()
    {
        const string runId = "018f0000-0000-7000-8000-000000000021";
        var targetBytes = 1_048_576;
        var result = SizedRepositoryAiResult(targetBytes, "\\\"result\\\":");
        Assert.Equal(targetBytes, Encoding.UTF8.GetByteCount(result));
        var run = $$"""{"run_id":"{{runId}}","state":"succeeded","scope_kind":"repository_selection","session_id":null,"node_id":null,"repository_id":"{{RepositoryId}}","error":null,"result":{{result}}}""";
        Assert.InRange(Encoding.UTF8.GetByteCount(run), targetBytes + 1, targetBytes + 4_096);
        ...
        await Expect(page.Locator("#session-ai-result")).ToContainTextAsync("AIによる解釈");
        await Expect(page.Locator("#session-ai-result")).ToContainTextAsync("\\\"result\\\":");
    }

    [Fact]
    [Trait("ValidationLane", "Nightly")]
    public async Task RepositoryAiStaleRestoreAndCancelCannotOverwriteAnalysisOnlyTraversalState()
    {
        const string runId = "018f0000-0000-7000-8000-000000000021";
        var restoreRequested = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var runReads = 0; var sessionReads = 0;
        using var temp = new MonitorTempDirectory();
        await using var host = await MonitorTestHost.StartAsync(temp, testOptions: Options(includeRepository: true));
        PlaywrightBrowserPath.ConfigureDefault(); using var playwright = await Playwright.CreateAsync();
        await using var browser = await playwright.Chromium.LaunchAsync(new BrowserTypeLaunchOptions { Headless = true });
        var page = await browser.NewPageAsync();
        await page.RouteAsync("**/api/local-monitor/v1/sessions", route => { sessionReads++; return route.FulfillAsync(Json(TwoSessionsAsync().GetAwaiter().GetResult())); });
        await page.RouteAsync("**/api/local-monitor/v1/settings/ai-readiness", route => route.FulfillAsync(Json("""{"readiness_state":"ready"}""")));
        await page.RouteAsync($"**/api/local-monitor/v1/ai/runs/{runId}/cancel", async route => { cancelRequested.TrySetResult(); await releaseCancel.Task; await route.FulfillAsync(Json($$"""{"run_id":"{{runId}}","state":"canceled"}""")); });
        await page.RouteAsync($"**/api/local-monitor/v1/ai/runs/{runId}", async route =>
        {
            runReads++;
            if (runReads == 1) { restoreRequested.TrySetResult(); await releaseRestore.Task; }
            await route.FulfillAsync(Json($$"""{"run_id":"{{runId}}","state":"running","scope_kind":"repository_selection","session_id":null,"node_id":null,"repository_id":"{{RepositoryId}}","error":null,"result":null}"""));
        });
        await page.GotoAsync(host.Url + $"/repositories/{RepositoryId}/sessions", new PageGotoOptions { WaitUntil = WaitUntilState.DOMContentLoaded });
        await page.Locator("#session-search").FillAsync("transient query");
        var stableSessionReads = sessionReads;

        await page.EvaluateAsync($"window.LocalMonitorV1History.push({{ analysis: '{runId}' }})");
        await restoreRequested.Task.WaitAsync(TimeSpan.FromSeconds(5));
        await page.GoBackAsync(); releaseRestore.TrySetResult();
        await page.WaitForTimeoutAsync(250);
        await Expect(page.Locator("#session-search")).ToHaveValueAsync("transient query");
        Assert.Equal(stableSessionReads, sessionReads);
        await Expect(page.Locator("#session-ai-dialog")).Not.ToBeVisibleAsync();

        await page.GoForwardAsync();
        var cancel = page.Locator("#session-ai-cancel");
        ...
        await page.GoBackAsync(); releaseCancel.TrySetResult();
        await page.WaitForTimeoutAsync(250);
        await Expect(page.Locator("#session-search")).ToHaveValueAsync("transient query");
        Assert.Equal(stableSessionReads, sessionReads);
        await Expect(page.Locator("#session-ai-dialog")).Not.ToBeVisibleAsync();
    ...
    public async Task ComparePreviewLocalizesExclusionsAndEscapeRetainsOnlyTransientSelection()
    {
        ...
        var previewResponse = ComparisonPreviewWithExclusion();
        using (var preview = JsonDocument.Parse(previewResponse))
        {
            Assert.Matches("^[0-9a-f]{64}$", preview.RootElement.GetProperty("selection_sha256").GetString());
            Assert.All(preview.RootElement.GetProperty("excluded").EnumerateArray(), excluded =>
                Assert.Equal(
                    ...
                    excluded.GetProperty("metadata").EnumerateObject().Select(property => property.Name)));
            Assert.Equal("session_archived", preview.RootElement.GetProperty("included")[0].GetProperty("metadata").GetProperty("archive_exclusion_reason").GetString());
        }
        var previewCalls = 0;
        string? previewBody = null;
        await page.RouteAsync($"**/api/local-monitor/v1/repositories/{RepositoryId}/comparisons/preview", route =>
        {
            previewCalls++;
            previewBody = route.Request.PostData;
            return route.FulfillAsync(Json(previewResponse));
        });
    ...
    [Fact]
    [Trait("ValidationLane", "Nightly")]
    public async Task ComparePreviewAcceptsTheNormativeProductionGolden()
    {
        using var temp = new MonitorTempDirectory();
        await using var host = await MonitorTestHost.StartAsync(temp, testOptions: Options(includeRepository: true));
        PlaywrightBrowserPath.ConfigureDefault();
        ...
        var page = await browser.NewPageAsync();
        await page.RouteAsync("**/api/local-monitor/v1/sessions", route => route.FulfillAsync(Json(ThreeSessionsAsync().GetAwaiter().GetResult())));
        var golden = await File.ReadAllTextAsync(Path.Combine(
            AppContext.BaseDirectory, "TestData", "LocalMonitorV1Comparison", "local-monitor-comparison-preview.response.json"));
        var preview = JsonNode.Parse(golden)!.AsObject();
        preview["requested"]![0]!["session_id"] = SessionId;
        preview["requested"]![1]!["session_id"] = ThirdSessionId;
        preview["requested"]![2]!["session_id"] = SecondSessionId;
        preview["included"]![0]!["session_id"] = SessionId;
        preview["included"]![1]!["session_id"] = SecondSessionId;
        preview["excluded"]![0]!["session_id"] = ThirdSessionId;
        golden = Canonical(preview);
        var obsolete = preview.DeepClone().AsObject();
        foreach (var item in obsolete["included"]!.AsArray().Concat(obsolete["excluded"]!.AsArray()))
        {
            var metadata = item!["metadata"]?.AsObject();
            if (metadata is null) continue;
            foreach (var name in new[] { "session_archive_revision", "assigned_repository_archive_state", "assigned_repository_archive_revision", "archive_exclusion_reason", "source_application_versions", "adapter_versions" })
                metadata.Remove(name);
        }
        var numericRevision = golden.Replace(
            $"\"projection_revision\":\"{new string('3', 64)}\"",
            $"\"projection_revision\":{new string('3', 64)}",
            StringComparison.Ordinal);
        var unicodeBoundary = preview.DeepClone().AsObject();
        unicodeBoundary["included"]![0]!["metadata"]!["source"] = string.Concat(Enumerable.Repeat("😀", 100));
        unicodeBoundary["included"]![0]!["metadata"]!["model"] = string.Concat(Enumerable.Repeat("😀", 200));
        unicodeBoundary["included"]![0]!["metadata"]!["metric_coverage"] = new JsonArray(string.Concat(Enumerable.Repeat("😀", 80)));
        var unicodeOversize = unicodeBoundary.DeepClone().AsObject();
        unicodeOversize["included"]![0]!["metadata"]!["source"] = string.Concat(Enumerable.Repeat("😀", 101));
        var responses = new[] { Canonical(obsolete), numericRevision, Canonical(unicodeBoundary), Canonical(unicodeOversize), golden };
        var previewCalls = 0;
        await page.RouteAsync($"**/api/local-monitor/v1/repositories/{RepositoryId}/comparisons/preview", route =>
            route.FulfillAsync(Json(responses[previewCalls++])));
        ...
        await page.Locator("#session-comparison-cancel").ClickAsync();
        await page.Locator("#session-compare-preview").ClickAsync();
        await Expect(page.Locator("#session-comparison-preview-status")).ToContainTextAsync("比較内容を確認してください");
        await page.Locator("#session-comparison-cancel").ClickAsync();
        await page.Locator("#session-compare-preview").ClickAsync();
        await Expect(page.Locator("#session-comparison-preview-status")).ToContainTextAsync("比較内容を読み込めませんでした");
        await page.Locator("#session-comparison-cancel").ClickAsync();
    ...
    public async Task AllSessionsPostsTheClosedRequestAndRendersADenseDirectOpenRow()
    {
        ...
        await page.Locator(".local-monitor-advanced-filters > summary").FocusAsync();
    ...
    public async Task RepositoryScopeAcceptsAnAutomaticAssignmentWithItsRetainedCandidate()
    {
        ...
        await Expect(page.Locator("#session-explorer-status")).ToHaveTextAsync("1件");
    ...
    public async Task KeyboardOrderReachesFiltersRowsOverflowPaginationAndCompareActions()
    {
        ...
        await TabToAsync(".local-monitor-session-filter-menu:has(#session-has-skill) > summary");
    ...
    public async Task SafeFiltersCursorBackAndReloadRoundTripWhileSearchAndModelRemainTransient()
    {
        ...
        await page.Locator(".local-monitor-session-filter-menu:has(#session-source) > summary").ClickAsync();
        await page.Locator(".local-monitor-session-filter-menu:has(#session-status) > summary").ClickAsync();
        await page.Locator("#session-status").SelectOptionAsync("completed");
        await page.Locator(".local-monitor-session-filter-menu:has(#session-status) > summary").ClickAsync();
        await page.Locator(".local-monitor-session-filter-menu:has(#session-has-skill) > summary").ClickAsync();
    ...
    public async Task InvalidOwnerSuccessResponseDoesNotOptimisticallyMutateOrRefreshTheRow()
    {
        ...
        await Expect(page.Locator("#session-explorer-status")).ToContainTextAsync("操作に失敗しました");
    ...
    public async Task ConfirmedArchiveAndAssignmentOwnerActionsUseExactIdentityRevisionThenRefresh()
    {
        ...
        await Expect(page.Locator("[data-session-row]")).ToHaveCountAsync(1);
        await Expect(page.Locator("#session-explorer-status")).ToHaveTextAsync(new Regex("^[0-9,]+件$"));
    ...
    public async Task AssignmentPickerUsesExactOwnerCollectionsCandidatesAndSameNameRepositoryIdentity()
    {
        ...
        Assert.Contains(host.Url + "/api/local-monitor/v1/repositories?archive_scope=active_only&limit=50", repositoryRequests);
        Assert.All(repositoryRequests.Where(request => request.Contains("archive_scope=include_archived", StringComparison.Ordinal)), request => Assert.Equal(
    ...
    public async Task AssignmentPickerKeepsTheExactSelectedRepositoryAcrossCandidatePagination()
    {
        ...
        await page.RouteAsync("**/api/local-monitor/v1/repositories?*", route =>
        {
            if (route.Request.Url.Contains("archive_scope=include_archived", StringComparison.Ordinal)) repositoryRequests++;
    ...
    public async Task CertificationPendingSummaryAndTokensKeepTheirDedicatedSharedVocabulary()
    {
        ...
        await Expect(page.Locator("[data-session-summary]")).ToContainTextAsync("未確認");
        await Expect(page.Locator("[data-session-tokens]")).ToContainTextAsync("未確認");
    ...
    public async Task DenseRow_ReflowsLongSummaryAndKeepsComponentStateDisclosureAccessible()
    {
        ...
        await Expect(row.Locator("[data-session-tokens]")).ToContainTextAsync("未確認");
        var rowBox = Assert.IsType<LocatorBoundingBoxResult>(await row.BoundingBoxAsync());
        Assert.True(rowBox.Height >= 64);
        Assert.True(await page.Locator("[data-session-row]").EvaluateAsync<bool>(
            "e => { const cell=e.querySelector('[data-session-open]'); return cell.scrollWidth <= cell.clientWidth + 1 && getComputedStyle(cell).whiteSpace === 'normal'; }"));
        ...
        Assert.True(summaryPanelBox.Height > 0 && summaryPanelBox.Height <= 768);
    ...
    public async Task EveryUnavailableFactFamilyKeepsItsDistinctSharedPresentation()
    {
        ...
        await Expect(identity).ToContainTextAsync("取得元: VS Code · 一部欠落");
        await Expect(identity).ToContainTextAsync("モデル: model-partial · 未確認");
        ...
        await Expect(timingDisclosure.Locator(".local-monitor-session-fact-panel"))
            .ToContainTextAsync("読取不可");
        await Expect(timingDisclosure.Locator(".local-monitor-session-fact-panel"))
            .ToContainTextAsync("整合性を確認できません");
    ...
    public async Task ArchivingASelectedSessionMarksTheExactIdExcludedWhenTheRowDisappears()
    {
        ...
        await Expect(page.Locator("#session-result-count")).ToBeFocusedAsync();
        Assert.Equal(2, collectionCalls);
    }
    ...
    private static string ComparisonPreview(char selection = '1', char revision = '2') => """
        {"schema_version":"local-monitor-comparison-preview.response.v1","valid":true,"selection_sha256":"1111111111111111111111111111111111111111111111111111111111111111","preview_revision":"2222222222222222222222222222222222222222222222222222222222222222","cohorts":{"a":{"label":"基準","requested_count":1,"included_count":1,"excluded_count":0},"b":{"label":"比較対象","requested_count":1,"included_count":1,"excluded_count":0}},"requested":[{"cohort":"a","request_ordinal":1,"session_id":"{SESSION}"},{"cohort":"b","request_ordinal":1,"session_id":"{SECOND}"}],"included":[{"cohort":"a","session_id":"{SESSION}","metadata":{"archive_state":"active","session_archive_revision":1,"assigned_repository_archive_state":"active","assigned_repository_archive_revision":1,"archive_exclusion_reason":null,"source":"synthetic","model":"test","projection_version":1,"completeness":"full","metric_coverage":[],"source_application_versions":[],"adapter_versions":[],"session_revision":1,"projection_revision":"3333333333333333333333333333333333333333333333333333333333333333"}},{"cohort":"b","session_id":"{SECOND}","metadata":{"archive_state":"active","session_archive_revision":1,"assigned_repository_archive_state":"active","assigned_repository_archive_revision":1,"archive_exclusion_reason":null,"source":null,"model":null,"projection_version":1,"completeness":"partial","metric_coverage":[],"source_application_versions":[],"adapter_versions":[],"session_revision":2,"projection_revision":"4444444444444444444444444444444444444444444444444444444444444444"}}],"excluded":[]}
    ...
    private static string SizedRepositoryAiResult(int targetBytes, string marker)
    {
        var limitations = new List<string> { marker };
        string Serialize() => JsonSerializer.Serialize(new
        {
            scope = new { kind = "repository_selection", repository_id = RepositoryId, anchor_id = RepositoryId },
            snapshot = new { snapshot_id = "018f0000-0000-7000-8000-000000000020", payload_sha256 = new string('a', 64) },
            summary = "summary",
            findings = new[] { new { finding_id = "f-1", title = "Finding", explanation = "Explanation", evidence_state = "supported", evidence_refs = new[] { $"/sessions/{SessionId}?node=node-a8a773d6614d5030f505ff195b452dd6" }, limitation = "none" } },
            improvement_suggestions = Array.Empty<object>(),
            limitations,
            provenance = new { provider = "github_copilot_sdk", model = "synthetic", configuration_sha256 = new string('b', 64), prompt_template_version = "v1", requested_at = "2026-08-31T00:00:00.0000000+00:00", started_at = "2026-08-31T00:00:01.0000000+00:00", completed_at = "2026-08-31T00:00:02.0000000+00:00", snapshot_id = "018f0000-0000-7000-8000-000000000020", snapshot_sha256 = new string('a', 64), coverage = new { included = 1, excluded = 0, content_available = true } },
        });
        var serialized = Serialize();
        while (Encoding.UTF8.GetByteCount(serialized) < targetBytes)
        {
            var remaining = targetBytes - Encoding.UTF8.GetByteCount(serialized);
            if (remaining <= 3)
            {
                limitations[^1] += new string('x', remaining);
            }
            else
            {
                limitations.Add(new string('x', Math.Min(16_384, remaining - 3)));
            }
    ...
    private static string ComparisonPreviewWithExclusion() => """
        {"schema_version":"local-monitor-comparison-preview.response.v1","valid":false,"selection_sha256":"1111111111111111111111111111111111111111111111111111111111111111","preview_revision":"2222222222222222222222222222222222222222222222222222222222222222","cohorts":{"a":{"label":"基準","requested_count":2,"included_count":1,"excluded_count":1},"b":{"label":"比較対象","requested_count":1,"included_count":0,"excluded_count":1}},"requested":[{"cohort":"a","request_ordinal":1,"session_id":"{SESSION}"},{"cohort":"a","request_ordinal":2,"session_id":"{THIRD}"},{"cohort":"b","request_ordinal":1,"session_id":"{SECOND}"}],"included":[{"cohort":"a","session_id":"{THIRD}","metadata":{"archive_state":"archived","session_archive_revision":1,"assigned_repository_archive_state":"active","assigned_repository_archive_revision":0,"archive_exclusion_reason":"session_archived","source":"synthetic","model":"test","projection_version":1,"completeness":"full","metric_coverage":[],"source_application_versions":[],"adapter_versions":[],"session_revision":3,"projection_revision":"5555555555555555555555555555555555555555555555555555555555555555"}}],"excluded":[{"cohort":"a","request_ordinal":1,"session_id":"{SESSION}","reason":"projection_unavailable","metadata":{"archive_state":"active","session_archive_revision":0,"assigned_repository_archive_state":"active","assigned_repository_archive_revision":0,"archive_exclusion_reason":null,"source":"synthetic","model":"test","projection_version":1,"completeness":"full","metric_coverage":[],"source_application_versions":null,"adapter_versions":null,"session_revision":1,"projection_revision":"3333333333333333333333333333333333333333333333333333333333333333"}},{"cohort":"b","request_ordinal":1,"session_id":"{SECOND}","reason":"projection_unavailable","metadata":{"archive_state":"active","session_archive_revision":0,"assigned_repository_archive_state":"active","assigned_repository_archive_revision":0,"archive_exclusion_reason":null,"source":null,"model":null,"projection_version":1,"completeness":"partial","metric_coverage":[],"source_application_versions":null,"adapter_versions":null,"session_revision":2,"projection_revision":"4444444444444444444444444444444444444444444444444444444444444444"}}]}
        """.Replace("{SESSION}", SessionId, StringComparison.Ordinal)
            .Replace("{SECOND}", SecondSessionId, StringComparison.Ordinal)
    ...
    private static string ExplorerArtifactPath(string name)
    {
        var directory = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "artifacts", "session-explorer"));
    ...
    private static async Task OpenAdvancedFiltersAsync(IPage page)
    {
        var closed = page.Locator(".local-monitor-advanced-filters:not([open]) > summary");
        if (await closed.CountAsync() > 0) await closed.ClickAsync();
    }
