...
public sealed class DoctorMigrationTests
{
    ...
    private const int CurrentMonitorSchemaVersion = SqliteSourceCompatibilityStore.MonitorSchemaVersion;
    ...
    public void HistoricalMonitorFixture_MigratesThroughCurrentMonitorVersionThenDoctorV1AndRestartsIdempotently(int version, string sourceCommit)
    {
        ...
        using (var firstRestart = database.Open())
        {
            ...
            AssertCurrentMonitorSchema(firstRestart);
        ...
        using (var secondRestart = database.Open())
        {
            ...
            AssertCurrentMonitorSchema(secondRestart);
        ...
        string failurePoint)
    {
        ...
        using (var connection = database.Open())
        {
            ...
            Assert.Equal((long)CurrentMonitorSchemaVersion, DoctorTestDatabase.Scalar(connection, "SELECT version FROM schema_version WHERE component='monitor';"));
    ...
    {
        Assert.Equal(
            ["doctor|1", $"monitor|{CurrentMonitorSchemaVersion}"],
    ...
    private static void AssertCurrentMonitorSchema(SqliteConnection connection)
    {
        Assert.Equal(
            [
