...
internal sealed class LocalAiAnalysisStoreV1
{
    ...
    internal void InsertSnapshot(LocalAiSnapshotV1 snapshot)
    {
        ...
        var expires=identity.Kind switch { LocalAiScopeKindV1.Session => (DateTimeOffset?)null, LocalAiScopeKindV1.Comparison => snapshot.ExpiresAt is { } bound ? Min(created.AddHours(24),bound) : throw new ArgumentException("local_ai_comparison_expiry_required"), LocalAiScopeKindV1.RepositorySelection=>snapshot.ExpiresAt is { } repositoryBound?Min(created.AddHours(24),repositoryBound):created.AddHours(24), _ => created.AddHours(24) };
        command.CommandText = """
            INSERT INTO local_ai_snapshots(snapshot_id,scope_kind,session_id,node_id,repository_id,comparison_id,anchor_id,payload_json,payload_sha256,evidence_index_json,evidence_index_sha256,retention_owner_token,created_at,expires_at)
    ...
    internal LocalAiSnapshotV1? ReadTransientSnapshot(string snapshotId)
    {
        ...
        command.CommandText="SELECT scope_kind,session_id,node_id,anchor_id,payload_json,evidence_index_json,repository_id,comparison_id,expires_at FROM local_ai_snapshots WHERE snapshot_id=$id AND scope_kind='repository_selection';";
    ...
    internal LocalAiRunV1 CreateRun(LocalAiRunRequestV1 request)
    {
        ...
        using (var snapshot = connection.CreateCommand())
        {
            ...
            requestExpiry=identity.Kind==LocalAiScopeKindV1.Session?null:identity.Kind is LocalAiScopeKindV1.Comparison or LocalAiScopeKindV1.RepositorySelection?Min(request.RequestedAt.AddHours(24),DateTimeOffset.Parse((string)snapshotExpiry,CultureInfo.InvariantCulture)):request.RequestedAt.AddHours(24);
        }
