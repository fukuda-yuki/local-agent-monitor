...
public sealed class SqliteRuntimeBackupService
{
    ...
    private static readonly string[] MigrationOrder = ["monitor", "session", "retention", "doctor", "alert_engine", "alert_lifecycle", "runtime_backup", "first_trace_navigation"];
    private readonly TimeProvider timeProvider;
    private readonly Action<string>? checkpoint;

    public SqliteRuntimeBackupService(TimeProvider? timeProvider = null) : this(timeProvider, null) { }
    ...
    private RuntimeBackupPreflightResult InitializeWithLease(string database)
    {
        ...
        var preflight = PreflightForMigration(database);
        if (!preflight.Success) return preflight;
        EnsureRuntimeBackupSchema(database);
        return PreflightForMigration(database);
    }
    ...
    public RuntimeBackupCreateResult CreateAndPublish(string databasePath, string outputPath)
    {
        ...
        var externalError = ValidateExternalState(database, scanRuntimeRoot: true);
    ...
    public RuntimeBackupPreflightResult PreflightForMigration(string databasePath)
    {
        ...
        try
        {
            ...
            if (!ValidateComponentShapes(database, versions)) return new(false, RuntimeBackupErrorCodes.RestoreIncompatible, versions);
    ...
    private RuntimeRestorePreview PreviewCore(Func<InspectionContext> inspect, string database)
    {
        ...
        try
        {
            ...
            var source = PreflightForMigration(context.StagePath);
            if (!source.Success) return PreviewFailure(RuntimeBackupErrorCodes.RestoreIncompatible, context.Result.ArchiveSha256);
            var current = PathEntryExists(database) ? PreflightForMigration(database) : new RuntimeBackupPreflightResult(true, null, new Dictionary<string, int>(), []);
            if (!current.Success) return PreviewFailure(RuntimeBackupErrorCodes.RestoreIncompatible, context.Result.ArchiveSha256);
            var externalError = ValidateExternalState(database, scanRuntimeRoot: false);
            ...
            var migratedExternalError = ValidateDatabaseExternalRawState(context.StagePath);
            if (migratedExternalError is not null) return PreviewFailure(migratedExternalError, context.Result.ArchiveSha256);
            _ = ValidateInstalledDatabase(context.StagePath);
            var comparison = PathEntryExists(database) ? CompareRetention(database, context.StagePath, context.Result.ArchiveSha256!) : RetentionComparison.Empty;
            if (!comparison.Success) return PreviewFailure(comparison.ErrorCode!, context.Result.ArchiveSha256);
            var result = new RuntimeRestorePreview(true, null, true, PathEntryExists(database), true, true,
    ...
    public RuntimeRestoreResult Restore(string bundlePath, string databasePath, RuntimeRestoreOptions options)
    {
        ...
        try
        {
            ...
            if (targetExisted)
            {
                ...
                var current = PreflightForMigration(database);
            ...
            var stagedExternalError = ValidateDatabaseExternalRawState(context.StagePath);
            if (stagedExternalError is not null) return RestoreFailure(stagedExternalError, context.Result.ArchiveSha256);
            var preflight = PreflightForMigration(context.StagePath);
            if (!preflight.Success) return RestoreFailure(RuntimeBackupErrorCodes.RestoreIncompatible, context.Result.ArchiveSha256);
            ...
            stagedExternalError = ValidateDatabaseExternalRawState(context.StagePath);
            if (stagedExternalError is not null) return RestoreFailure(stagedExternalError, context.Result.ArchiveSha256);
            if (targetExisted)
            {
                comparison = CompareRetention(database, context.StagePath, context.Result.ArchiveSha256!);
                if (!comparison.Success) return RestoreFailure(comparison.ErrorCode!, context.Result.ArchiveSha256);
                if (comparison.NonTerminalCount > 0 && (!options.AllowResurrection || !FixedEquals(options.ConfirmationDigest, comparison.ConfirmationDigest)))
                    return RestoreFailure(RuntimeBackupErrorCodes.RestoreResurrectionBlocked, context.Result.ArchiveSha256, comparison);
            }

            if (targetExisted && !ReconcileTerminal(database, context.StagePath, comparison))
                return RestoreFailure(RuntimeBackupErrorCodes.RestoreTombstoneReconcileFailed, context.Result.ArchiveSha256, comparison);
            ...
            if (targetExisted)
            {
                ...
                if (string.IsNullOrWhiteSpace(prePath))
                {
                    ...
                    if (!TryFullFile(candidate, mustExist: false, out prePath))
                        return RestoreFailure(RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe, context.Result.ArchiveSha256, comparison);
                }
                preRestore = CreateArchive(database, prePath, scanRuntimeRoot: false);
                if (!preRestore.Success) return RestoreFailure(preRestore.ErrorCode!, context.Result.ArchiveSha256, comparison);
            }
            ...
            if (HasActiveSqliteSidecar(context.StagePath))
                throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreFailed);
            var stagedFacts = ValidateInstalledDatabase(context.StagePath);
            if (!HasMatchingRestoreReceipt(context.StagePath, operationId, context.Result.ArchiveSha256!))
                throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreFailed);
            ...
            if (HasActiveSqliteSidecar(database) || !HashEquals(database, stagedHash))
                throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreFailed);
            ValidateInstalledDatabase(database, stagedFacts);
            if (!HasMatchingRestoreReceipt(database, operationId, context.Result.ArchiveSha256!))
                throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreFailed);
            ...
            if (!HashEquals(database, stagedHash) || !HasMatchingRestoreReceipt(database, operationId, context.Result.ArchiveSha256!))
                throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreFailed);
            ...
            return new(true, null, RuntimeBackupContractVersions.RestoreResult, context.Result.ArchiveSha256,
                ...
                comparison.TerminalCount, comparison.NonTerminalCount, "database_ready", DoctorCheck(database), RuntimeBackupWarnings.All);
        }
        catch (Exception exception) when (exception is RuntimeBackupException or SqliteException or InvalidOperationException or InvalidDataException or JsonException or InvalidCastException or FormatException or OverflowException || IsIo(exception))
        {
            preserveRecoveryArtifacts = false;
            ...
            var rollbackSucceeded = swapped
                ...
                : restoreJournal is null || RecoverInterruptedRestore(database);
            ownership?.Dispose();
            ...
            return RestoreFailure(swapped ? (rollbackSucceeded ? RuntimeBackupErrorCodes.RestoreRolledBack : RuntimeBackupErrorCodes.RestoreRollbackFailed) : preSwapCode,
    ...
    private RuntimeBackupCreateResult CreateArchive(string databasePath, string outputPath, bool scanRuntimeRoot)
    {
        ...
        try
        {
            ...
            var cursorsAtStart = ReadProjectionCursors(databasePath);
            var startExternalError = ValidateExternalState(databasePath, scanRuntimeRoot);
            ...
            var snapshotPreflight = PreflightForMigration(snapshot);
            if (!snapshotPreflight.Success) return CreateFailure(RuntimeBackupErrorCodes.RestoreIncompatible);
            var capturedExternalError = ValidateDatabaseExternalRawState(snapshot);
            if (capturedExternalError is not null) return CreateFailure(capturedExternalError);
            var cursorsAtEnd = ReadProjectionCursors(databasePath);
            var endExternalError = ValidateExternalState(databasePath, scanRuntimeRoot, [snapshot, snapshotOwner.MarkerPath]);
    ...
    private InspectionContext InspectToStage(FileStream file, string stageBasePath, string? requestedStagePath = null)
    {
        ...
        try
        {
            ...
            var databaseHash = HashFile(stage);
            if (manifest.DatabaseSize != new FileInfo(stage).Length || !FixedEquals(databaseHash, manifest.DatabaseSha256)) throw new RuntimeBackupException(RuntimeBackupErrorCodes.ChecksumMismatch);
            ValidateDatabaseMatchesManifest(stage, manifest);
            var stagedExternalError = ValidateDatabaseExternalRawState(stage);
        ...
        {
            if (stageOwner is not null) _ = stageOwner.Cleanup();
            else DeleteStageFiles(stage);
            throw;
        }
    }
        ...
        DateTimeOffset createdAt)
    {
        ...
        var cursors = ReadProjectionCursors(snapshotPath);
        var retention = ReadRetentionSummary(snapshotPath);
    ...
    private static string ReadJournalMode(string path)
    {
        ...
        return command.ExecuteScalar() as string ?? throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreIncompatible);
    }

    private static void OnlineSnapshot(string sourcePath, string destinationPath)
    {
    ...
    private static void ValidateDatabaseMatchesManifest(string path, RuntimeBackupManifestData manifest)
    {
        ...
        if (!ReadRowCounts(connection).OrderBy(item => item.Key, StringComparer.Ordinal).SequenceEqual(manifest.RowCounts.OrderBy(item => item.Key, StringComparer.Ordinal)))
            throw new RuntimeBackupException(RuntimeBackupErrorCodes.ManifestInvalid);
        if (!ValidateComponentShapes(path, manifest.ComponentVersions)) throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreIncompatible);
        ValidateRetentionPreflightBounds(connection);
        if (manifest.ComponentVersions.ContainsKey("retention"))
        {
            ValidateRetentionCoverage(path);
            ValidateRetentionInvariants(path);
        }
        if (!ReadProjectionCursors(path).OrderBy(item => item.Key, StringComparer.Ordinal).SequenceEqual(manifest.ProjectionCursors.OrderBy(item => item.Key, StringComparer.Ordinal)))
            throw new RuntimeBackupException(RuntimeBackupErrorCodes.ManifestInvalid);
        var retention = ReadRetentionSummary(path);
    ...
    private static InstalledDatabaseFacts ValidateInstalledDatabase(string path, InstalledDatabaseFacts? expected = null)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly);
        ValidateIntegrity(connection);
        ValidateSchemaMetadataBounds(connection);
        ...
        if (!ValidateComponentShapes(path, versions)) throw new RuntimeBackupException(RuntimeBackupErrorCodes.RestoreIncompatible);
        ValidateRetentionPreflightBounds(connection);
        ValidateRetentionCoverage(path);
        var cursors = ReadProjectionCursors(path);
        var retention = ReadRetentionSummary(path);
        ValidateRetentionInvariants(path);
    ...
    private static void ValidateRetentionInvariants(string path)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly);
    ...
    private static bool ValidateComponentShapes(string path, IReadOnlyDictionary<string, int> versions)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly);
        ...
        if (versions.ContainsKey("runtime_backup") && !RuntimeBackupSchemaV1.IsValid(connection, null)) return false;
    ...
    private static bool ValidateExecutableObjects(SqliteConnection connection, IReadOnlyDictionary<string, int> versions)
    {
        ...
        if (!ValidateIndexesAreNonExecutable(connection)) return false;
    ...
    private static bool ValidateIndexesAreNonExecutable(SqliteConnection connection)
    {
        ...
        foreach (var index in indexes)
        {
            using (var list = connection.CreateCommand())
            {
                ...
                if (!reader.Read() || reader.GetFieldType(0) != typeof(long) || reader.GetInt64(0) != 0
    ...
    private static bool ValidateOwnedComponentNamespaces(SqliteConnection connection, IReadOnlyDictionary<string, int> versions)
    {
        ...
        var prefixes = new[] { "doctor_", "alert_", "runtime_backup_", "first_trace_" };
        using var command = connection.CreateCommand();
    ...
    { using var connection = Open(path, readOnly ? SqliteOpenMode.ReadOnly : SqliteOpenMode.ReadWrite); return ReadComponentVersions(connection); }
    ...
    private static Dictionary<string, long> ReadRowCounts(string path) { using var connection = Open(path, SqliteOpenMode.ReadOnly); return ReadRowCounts(connection); }
    ...
    private static Dictionary<string, long?> ReadProjectionCursors(string path)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly); var result = new Dictionary<string, long?>(StringComparer.Ordinal);
    ...
    private static RuntimeBackupRetentionSummary ReadRetentionSummary(string path)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly);
    ...
    private static string? ValidateDatabaseExternalRawState(string databasePath)
    {
        try
        {
    ...
    private string? ValidateExternalState(string databasePath, bool scanRuntimeRoot, IReadOnlyCollection<string>? additionalAllowedFiles = null)
    {
        try
        {
            if (HasHotRollbackJournal(databasePath)) return RuntimeBackupErrorCodes.ExternalRuntimeStateActive;
            var rawError = ValidateDatabaseExternalRawState(databasePath);
            if (rawError is not null) return rawError;
            var proposal = Path.Combine(Path.GetDirectoryName(databasePath)!, "proposal-apply");
            if (PathEntryExists(proposal))
            {
                var proposalAttributes = File.GetAttributes(proposal);
                if ((proposalAttributes & FileAttributes.Directory) == 0
                    || (proposalAttributes & FileAttributes.ReparsePoint) != 0) return RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe;
                var entries = Directory.EnumerateFileSystemEntries(proposal, "*", SearchOption.TopDirectoryOnly)
                    .Take(RuntimeBackupLimits.MaximumInventoryItems + 1).ToArray();
                if (entries.Length > RuntimeBackupLimits.MaximumInventoryItems) return RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe;
                foreach (var path in entries)
                {
                    var attributes = File.GetAttributes(path);
                    if ((attributes & FileAttributes.ReparsePoint) != 0) return RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe;
                    if ((attributes & FileAttributes.Directory) != 0) return RuntimeBackupErrorCodes.ExternalRuntimeStateActive;
                    if (!string.Equals(Path.GetFileName(path), "apply-root-map.json", StringComparison.Ordinal)
                        || !ValidateProposalRootMap(path)) return RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe;
                }
            }
            ...
            foreach (var entry in entries)
            {
                ...
                else if (!allowedFiles.Contains(name) && !(additionalAllowedFiles?.Any(path => PathEquals(path, entry)) ?? false))
                {
                    if (!IsRecognizedOperatorBackup(entry)) return RuntimeBackupErrorCodes.ExternalRuntimeStateUnknown;
                }
            }
            return null;
        }
    ...
    private static bool HasHotRollbackJournal(string databasePath)
    {
        var journal = databasePath + "-journal";
        if (!PathEntryExists(journal)) return false;
        if (!IsRegularControlFile(journal)) return true;
        try
        {
            using var stream = new FileStream(journal, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete);
            Span<byte> header = stackalloc byte[8];
            var read = stream.Read(header);
            return read > 0 && header[..read].IndexOfAnyExcept((byte)0) >= 0;
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException or System.Security.SecurityException) { return true; }
    }
    ...
    private static string ReadProposalApplyState(string directory)
    {
        ...
        if (entries.Length != 1 || !string.Equals(Path.GetFileName(entries[0]), "apply-root-map.json", StringComparison.Ordinal))
            throw new RuntimeBackupException(RuntimeBackupErrorCodes.ExternalRuntimeStateActive);
        if (!ValidateProposalRootMap(entries[0]) || ReadBoundedControlFile(entries[0], 256 * 1024) is not { } bytes)
            throw new RuntimeBackupException(RuntimeBackupErrorCodes.ExternalRuntimeStateUnsafe);
    ...
    private static RetentionComparison CompareRetention(string currentPath, string stagedPath, string archiveHash)
    {
        try
        {
            using var current = Open(currentPath, SqliteOpenMode.ReadOnly); using var staged = Open(stagedPath, SqliteOpenMode.ReadOnly);
    ...
    private static bool ReconcileTerminal(string currentPath, string stagedPath, RetentionComparison expected)
    {
        try
        {
            using var current = Open(currentPath, SqliteOpenMode.ReadOnly); using var staged = Open(stagedPath, SqliteOpenMode.ReadWrite);
            using var currentTransaction = current.BeginTransaction(deferred: true);
    ...
    private void MigrateStaging(string path, IReadOnlyDictionary<string, int> versions)
    {
        ...
        _ = new SqliteFirstTraceNavigationStore(path);
    }

    private static void ValidateRetentionCoverage(string path)
    {
        using var connection = Open(path, SqliteOpenMode.ReadOnly);
    ...
    private static bool HasMatchingRestoreReceipt(string path, string operationId, string archiveHash)
    {
        try
        {
            using var connection = Open(path, SqliteOpenMode.ReadOnly);
    ...
    private static bool ValidateCommittedTarget(string target, string stagePath, RestoreJournal journal)
    {
        try
        {
            if (journal.InstalledSha256 is null || HasActiveSqliteSidecar(target) || PathEntryExists(stagePath)
                ...
                || !HasMatchingRestoreReceipt(target, journal.OperationId, journal.ArchiveSha256)) return false;
            ValidateInstalledDatabase(target);
            return true;
        }
        catch (Exception exception) when (exception is RuntimeBackupException or SqliteException or InvalidOperationException || IsIo(exception)) { return false; }
    }
    ...
    private static bool RecoverTransientFiles(string directory, string? databaseFileName)
    {
        try
        {
                     ...
                     })
            {
                ...
                {
                    if (markers.Count >= RuntimeBackupLimits.MaximumInventoryItems) return false;
                    markers.Add(marker);
                }
            }
        ...
        out bool sqlite)
    {
        ...
        string? token = null;
        if (name.StartsWith(".runtime-backup-", StringComparison.Ordinal)
            && name.EndsWith(".partial.owner.v1", StringComparison.Ordinal))
        {
            token = name[".runtime-backup-".Length..^".partial.owner.v1".Length];
        }
        else if (name.StartsWith(".runtime-backup-inspect-", StringComparison.Ordinal)
                 && name.EndsWith(".sqlite.owner.v1", StringComparison.Ordinal))
        {
        ...
        if (token is not { Length: 32 } || token.Any(character => character is not (>= '0' and <= '9' or >= 'a' and <= 'f')))
            return false;
    ...
    private static void ValidateInstalledOrLegacy(string path) { using var connection = Open(path, SqliteOpenMode.ReadOnly); ValidateIntegrity(connection); }
    private static string DoctorCheck(string path) { using var connection = Open(path, SqliteOpenMode.ReadOnly); return TableExists(connection, "doctor_verifications") ? "doctor_store_ready" : "doctor_store_not_applicable"; }
    ...
    private static SqliteConnection Open(string path, SqliteOpenMode mode) { var connection = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = path, Mode = mode, Pooling = false, DefaultTimeout = 5 }.ToString()); connection.Open(); using var command = connection.CreateCommand(); command.CommandText = "PRAGMA busy_timeout=5000; PRAGMA foreign_keys=ON;"; command.ExecuteNonQuery(); return connection; }
    ...
    private static bool IsHostNativeAbsoluteLocalPath(string? path)
    {
    ...
    private static bool HasWindowsReservedSegment(string path)
    {
        ...
        foreach (var segment in path[root.Length..].Split(['\\', '/'], StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (basename.Length == 4
                ...
                && basename[3] is >= '1' and <= '9') return true;
        }
        return false;
    }
    ...
    private static bool IsReservedTransientArtifactName(string path)
    {
        ...
        return name.StartsWith(".runtime-backup-", StringComparison.Ordinal)
                   && name.EndsWith(".partial.owner.v1", StringComparison.Ordinal)
            || name.StartsWith(".runtime-backup-inspect-", StringComparison.Ordinal)
                   && name.EndsWith(".sqlite.owner.v1", StringComparison.Ordinal)
            || name.StartsWith(".", StringComparison.Ordinal)
                   && name.EndsWith(".online-snapshot.owner.v1", StringComparison.Ordinal);
    }
    ...
    private sealed class OwnedRuntimeBackupTransient(
        ...
        FileStream marker)
    {
        private FileStream? markerStream = marker;

        internal string MarkerPath { get; } = markerPath;
