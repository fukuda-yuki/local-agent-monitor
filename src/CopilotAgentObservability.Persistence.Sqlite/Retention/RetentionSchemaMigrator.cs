    ...
    internal static void Apply(SqliteConnection connection, SqliteTransaction transaction)
    {
        ...
        Execute(connection, transaction, """
            CREATE TABLE IF NOT EXISTS retention_items (
                ...
                retry_exhausted INTEGER NOT NULL DEFAULT 0 CHECK (retry_exhausted IN (0,1)), deleted_at TEXT NULL, adapter_coverage_version INTEGER NOT NULL CHECK (adapter_coverage_version = 0),
        ...
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_adapter_coverage(store_kind TEXT PRIMARY KEY CHECK (store_kind IN ('session_event_content','raw_record','analysis_run_raw','sensitive_bundle','analysis_sdk_directory')), coverage_version INTEGER NOT NULL CHECK (coverage_version = 0));");
