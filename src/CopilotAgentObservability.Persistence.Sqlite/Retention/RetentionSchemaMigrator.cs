    ...
    internal static void Apply(SqliteConnection connection, SqliteTransaction transaction)
    {
        ...
        Execute(connection, transaction, """
            CREATE TABLE IF NOT EXISTS retention_items (
                item_id TEXT PRIMARY KEY, store_instance_id TEXT NOT NULL, store_kind TEXT NOT NULL CHECK (store_kind IN ('session_event_content','raw_record','analysis_run_raw','sensitive_bundle','analysis_sdk_directory')),
                source_item_id TEXT NOT NULL, owner_reference TEXT NULL, private_locator TEXT NULL,
                captured_at TEXT NOT NULL, expires_at TEXT NULL, policy_id TEXT NOT NULL, policy_version INTEGER NOT NULL,
            ...
            """);
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_tombstones(item_id TEXT PRIMARY KEY, receipt_at TEXT NULL, deleted_at TEXT NULL, FOREIGN KEY(item_id) REFERENCES retention_items(item_id));");
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_capture_journal(item_id TEXT PRIMARY KEY, phase TEXT NOT NULL, durable_cursor TEXT NULL, FOREIGN KEY(item_id) REFERENCES retention_items(item_id));");
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_leases(item_id TEXT NOT NULL, lease_kind TEXT NOT NULL CHECK (lease_kind IN ('access','operation')), owner TEXT NOT NULL, expires_at TEXT NOT NULL, generation INTEGER NOT NULL CHECK (generation > 0), PRIMARY KEY(item_id, lease_kind), FOREIGN KEY(item_id) REFERENCES retention_items(item_id));");
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_delete_journal(item_id TEXT PRIMARY KEY, durable_cursor TEXT NULL, intent_at TEXT NULL, FOREIGN KEY(item_id) REFERENCES retention_items(item_id));");
        Execute(connection, transaction, "CREATE TABLE IF NOT EXISTS retention_adapter_coverage(store_kind TEXT PRIMARY KEY, coverage_version INTEGER NOT NULL);");
