...
internal sealed class LocalWorkspaceSessionDetailSnapshotContributor : ILocalWorkspaceSessionDetailSnapshotContributor
{
        ...
        SkillProjectionCurrentInvocationProjection? skillProjection, string registryIdentity, CancellationToken token)
    {
        ...
        var statements = new List<string>
        {
            ...
            "SELECT m.* FROM monitor_spans m WHERE EXISTS(SELECT 1 FROM session_runs r WHERE r.session_id=$session_id AND r.trace_id=m.trace_id COLLATE BINARY) ORDER BY m.raw_record_id,m.span_ordinal",
            "SELECT r.id,r.source,r.trace_id,r.received_at,r.schema_version,r.retention_owner_token FROM raw_records r WHERE EXISTS(SELECT 1 FROM monitor_spans m JOIN session_runs run ON run.session_id=$session_id AND run.trace_id=m.trace_id COLLATE BINARY WHERE m.raw_record_id=r.id) ORDER BY r.id",
            "SELECT f.* FROM local_workspace_span_facts f JOIN monitor_spans m ON m.raw_record_id=f.raw_record_id AND m.span_ordinal=f.span_ordinal WHERE EXISTS(SELECT 1 FROM session_runs r WHERE r.session_id=$session_id AND r.trace_id=m.trace_id COLLATE BINARY) ORDER BY f.raw_record_id,f.span_ordinal",
