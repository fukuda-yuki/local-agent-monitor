...
internal static class RawLocalReceiverHandler
{
    ...
    public static RawLocalReceiverResponse Handle(RawLocalReceiverRequest request)
    {
        ...
        try
        {
            payloadJson = contentType switch
            {
                JsonContentType => Encoding.UTF8.GetString(request.Body),
                ProtobufContentType => OtlpProtobufTraceConverter.ConvertTraceRequestToRawOtlpJson(request.Body),
                _ => throw new UnsupportedContentTypeException(),
            };

            EnsurePayloadContainsSpan(payloadJson);
    ...
    private static void EnsurePayloadContainsSpan(string payloadJson)
    {
