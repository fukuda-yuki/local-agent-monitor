...
#pragma warning restore GHCP001
    ...
    internal static string BuildPrompt(LocalAiProviderRequestV1 request)
    {
        ...
        if (request.Snapshot.ScopeKind == "repository_selection")
            builder.AppendLine("Summarize only the supplied frozen repository facts and evidence. Do not explore, infer, or request any session outside this snapshot. Do not recalculate deterministic facts, claim effects or causality, score quality, rank or prioritize, classify improvement or regression, or invent facts.");
