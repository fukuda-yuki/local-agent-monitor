...
internal static class LocalMonitorV1SessionDetailRoutes
{
    ...
    private static async Task HandleContent(HttpContext context,ILocalRepositorySessionDetailSnapshotService service,ILocalWorkspaceNodeContentReader reader,Action<LocalMonitorNodeContentRoutePhase>? checkpoint)
    {
        ...
        try
        {
            ...
            if(read.Disposition!=LocalWorkspaceNodeContentReadDisposition.Granted){var mapped=read.Disposition switch{LocalWorkspaceNodeContentReadDisposition.NotCaptured=>(404,"raw_content_not_captured"),LocalWorkspaceNodeContentReadDisposition.Oversized=>(413,"raw_content_too_large"),LocalWorkspaceNodeContentReadDisposition.Busy=>(503,"persistence_busy"),LocalWorkspaceNodeContentReadDisposition.Stale=>(409,"workspace_snapshot_stale"),LocalWorkspaceNodeContentReadDisposition.Expired=>(410,"raw_content_expired"),LocalWorkspaceNodeContentReadDisposition.Deleted=>(410,"raw_content_deleted"),LocalWorkspaceNodeContentReadDisposition.ReadDenied=>(403,"raw_content_read_denied"),_=>(503,"local_monitor_ui_unavailable")};await Error(context,mapped.Item1,mapped.Item2);return;}
    ...
    private static async Task Handle(HttpContext context,ILocalRepositorySessionDetailSnapshotService service,byte[] key,Kind kind)
    {
        ...
        catch(LocalWorkspaceSessionDetailException e){await Error(context,e.Error switch{"session_not_found" or "execution_not_found" or "node_not_found"=>404,"local_monitor_ui_unavailable"=>503,_=>409},e.Error);}
        catch(LocalRepositoryScopeSnapshotException){await Error(context,503,"persistence_busy");}
...
internal enum LocalMonitorNodeContentRoutePhase
{
    BeforeRetentionGrant,
    AfterCommittedGrantBeforeReference,
    WhileContentReferenceHeld,
    AfterBytesSelectedBeforeSeal,
