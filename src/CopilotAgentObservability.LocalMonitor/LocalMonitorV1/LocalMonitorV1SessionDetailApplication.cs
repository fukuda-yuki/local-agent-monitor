...
internal static class LocalMonitorV1SessionDetailApplication
{
    ...
    private static void Tokens(Utf8JsonWriter w,LocalWorkspaceTokenFacts t){w.WritePropertyName("tokens");w.WriteStartObject();w.WriteString("authority",t.Authority);w.WriteString("state",t.State);w.WriteNumber("available_execution_count",t.AvailableExecutionCount);w.WriteNumber("total_execution_count",t.TotalExecutionCount);Value(w,"input",t.Input);Value(w,"output",t.Output);Value(w,"total",t.Total);Value(w,"reasoning",t.Reasoning);Value(w,"cache_read",t.CacheRead);Value(w,"cache_creation",t.CacheCreation);Value(w,"new_input",t.NewInput);Value(w,"cache_read_ratio_basis_points",t.CacheReadRatioBasisPoints);LocalMonitorV1CollectionApplication.TokenObservations(w,t);w.WriteEndObject();}
