...
internal static class LocalMonitorV1SessionDetailApplication
{
    ...
    private static void NodeItem(Utf8JsonWriter w,LocalWorkspaceNodeDetail n,LocalWorkspaceSessionDetailContribution d,bool technical=false)
    {
        ...
        if(technical){w.WritePropertyName("technical_references");w.WriteStartObject();w.WriteString("source_kind",n.SourceKind);w.WriteString("source_identity",n.SourceIdentity);Nullable(w,"trace_id",n.TraceId);Nullable(w,"span_id",n.SpanId);Nullable(w,"event_id",n.EventId);w.WriteEndObject();Metadata(w,n);}
        w.WriteEndObject();
    }
    ...
    private static void Metadata(Utf8JsonWriter w,LocalWorkspaceNodeDetail n){w.WritePropertyName("metadata");w.WriteStartObject();w.WriteString("kind",n.Kind);switch(n.Kind){case "event":w.WritePropertyName("event_name");w.WriteStartObject();w.WriteString("state",n.NameState);Nullable(w,"text",n.NameText);w.WriteEndObject();w.WritePropertyName("source_time");w.WriteStartObject();w.WriteString("state",n.TimeAuthority);Nullable(w,"value",Instant(n.StartUtcTicks));w.WriteEndObject();w.WritePropertyName("content");w.WriteStartObject();w.WriteString("state","not_observed");w.WriteEndObject();SourceReferences(w,n);break;case "execution":break;default:SourceReferences(w,n);break;}w.WriteEndObject();}
