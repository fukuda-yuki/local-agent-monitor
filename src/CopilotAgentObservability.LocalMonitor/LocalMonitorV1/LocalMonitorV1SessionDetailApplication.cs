...
internal static class LocalMonitorV1SessionDetailApplication
{
    ...
    private static void Metadata(Utf8JsonWriter w,LocalWorkspaceNodeDetail n,LocalWorkspaceSessionDetailContribution d)
    {
        ...
        {
            case "tool":
                NodeReferenceFact(w,"caller",null);ScalarFact(w,"lifecycle",n.Lifecycle);ScalarFact(w,"status",n.Status);StateFact(w,"exit","not_observed");
                ScalarFact(w,"mcp_server_identity",null);ScalarFact(w,"mcp_server_name",null);ScalarFact(w,"mcp_tool_name",n.NameState=="recorded"?n.NameText:null);
                ContentFact(w,"input",n,d,"tool_input");ContentFact(w,"result",n,d,"tool_result");ContentFact(w,"error",n,d,"error_message");
                NodeReferencesFact(w,"retry",[]);NodeReferencesFact(w,"recovery",[]);w.WritePropertyName("child_activity");Activity(w,n.Activity);SourceReferences(w,n);break;
            case "skill":
                w.WriteString("current_valid_state","unavailable");ScalarFact(w,"source",null);ScalarFact(w,"trigger",null);ScalarFact(w,"inventory_reference",null);ScalarFact(w,"historical_snapshot_reference",null);break;
            case "subagent":
                w.WritePropertyName("lifecycle");w.WriteStartObject();foreach(var state in new[]{"selected","started","completed","failed","deselected"})StateFact(w,state,n.Lifecycle==state?"recorded":"not_observed");w.WriteEndObject();
                ContentFact(w,"input",n,d,"subagent_input");w.WritePropertyName("activity");Activity(w,n.Activity);w.WritePropertyName("tokens");Tokens(w,n.Tokens);CountFact(w,"children",n.ChildCount);SourceReferences(w,n);break;
            case "error":
                ScalarFact(w,"error_code",null);ContentFact(w,"message",n,d,"error_message");ScalarFact(w,"status",n.Status);SourceReferences(w,n);break;
            case "permission":
                ScalarFact(w,"decision",null);StateFact(w,"wait","not_observed");SourceReferences(w,n);break;
            case "event":
                ScalarFact(w,"event_name",n.NameState=="recorded"?n.NameText:null);
    ...
    private static void ScalarFact(Utf8JsonWriter w,string name,string? value,string valueName="value"){w.WritePropertyName(name);w.WriteStartObject();w.WriteString("state",value is null?"not_observed":"recorded");Nullable(w,valueName,value);w.WriteEndObject();}
    ...
    private static void NodeReferenceFact(Utf8JsonWriter w,string name,string? nodeId){w.WritePropertyName(name);w.WriteStartObject();w.WriteString("state",nodeId is null?"not_observed":"recorded");Nullable(w,"node_id",nodeId);w.WriteEndObject();}
    private static void NodeReferencesFact(Utf8JsonWriter w,string name,string[] nodeIds){w.WritePropertyName(name);w.WriteStartObject();w.WriteString("state",nodeIds.Length==0?"not_observed":"recorded");w.WritePropertyName("node_ids");JsonSerializer.Serialize(w,nodeIds);w.WriteEndObject();}
    ...
    private static LocalWorkspaceActivityFacts SummaryActivity(LocalWorkspaceProjectionRow s)=>s.Completeness=="full"?CompleteZeroActivity(s.Activity):s.Activity;
    private static LocalWorkspaceActivityFacts CompleteZeroActivity(LocalWorkspaceActivityFacts a){var zero=new LocalWorkspaceFact<long>("recorded",0);return new(a.Skill,a.Tool.State=="not_observed"?zero:a.Tool,a.Subagent.State=="not_observed"?zero:a.Subagent,a.Error.State=="not_observed"?zero:a.Error,a.Retry.State=="not_observed"?zero:a.Retry);}
