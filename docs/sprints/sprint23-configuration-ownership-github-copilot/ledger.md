...
| Issue #66-#67 materialization contract correction (Tasks 04a–04c) | #66-#67 | Reopened (#66 gate; contract correction; implementation in staged task closeout) | `ddcd342`, `8fd5fda`, `729c252` | Exact read-only `ISetupProcessEnvironment.Get(string name)` process boundary remains separate from current-user environment writes. Private-plan `desired_state` is a required schema-v1 closed inline/tagged union: inline is canonical for historical/generic non-tagged records, while tagged `jsonc_owned_values_v1` is restricted to `SetupTargetKind.Json` `github-copilot` VS Code Default Profile labels with 1..2048 UTF-16-unit strings and lowercase hashes. Task 04b now records the separate production-serializer `private-plan.v1.json` fixture and byte-identical write/close/reopen proof; existing committed `ownership-ledger.v1.json` remains unchanged/restart-readable. VS Code reads are bounded to 1 MiB plus one sentinel; complete JSONC bytes are transient during bounded planning/apply only, discarded before persistence, and recovery uses expected hashes plus flushed backup without rematerialization. | Review history: C0/I4/M1 → C0/I1/M0 → PASS C0/I0/M0 | Task 04c remains pending; #66 T2 gate is reopened and unresolved interfaces are T4/T5/T6/T7; migration N/A; public HTTP/proxy/UI N/A |
