...
| Transaction and rollback | #66 | In progress through Task 5d3a | `2d88eff..4659c22` | journal/store, exact artifact reuse, terminal/active Apply recovery, file rollback recovery, apply/compensation, and lock-lifetime evidence above | Tasks through 5d3a and Lock A-C PASS/APPROVED | environment/mixed rollback recovery and normal rollback producer remain |
...
| Stale plan/apply and rollback | Barrier-controlled hash changes without sleeps | Apply preflight and post-mutation full-target verification covered by SetupApplyTests 55/55; file rollback restart recovery covers stale/no-op/third-party/unavailable classification and retry admission in SetupRecoveryTests 212/212; environment/mixed and normal rollback remain pending |
...
| Crash recovery | Pre-file/per-env-member mutation and restore intents; deterministic faults before/during/after each step; close/reopen recovery classifies prior/desired/partial/foreign states | Apply terminal, notification-only, file active, environment active, mixed recovery, and interrupted file rollback covered by SetupRecoveryTests 212/212; Recovery+Journal 359/359; actual Apply producer→reopened Recovery consumer passes; environment/mixed rollback remains pending |
...
- Environment/mixed rollback recovery and the normal rollback producer are not
  yet implemented or independently reviewed; Task 5d3a evidence is file-only.
