...
| Transaction and rollback | #66 | In progress through Task 5d3b2b1 | `2d88eff..c4d2580` | journal/store, exact artifact reuse, terminal/active Apply recovery, file, single-environment, mixed active/committed rollback recovery, committed lagging-ledger reconciliation, apply/compensation, and lock-lifetime evidence above | Tasks through 5d3b2b1 and Lock A-C PASS/APPROVED | notification ambiguity remains; normal rollback producer remains |
...
| Stale plan/apply and rollback | Barrier-controlled hash changes without sleeps | Apply preflight and post-mutation full-target verification covered by SetupApplyTests 55/55; file, single-environment, mixed rollback restart recovery, and committed lagging-ledger reconciliation cover stale/no-op/third-party/unavailable classification and retry admission in SetupRecoveryTests 266/266; normal rollback remains pending |
...
| Crash recovery | Pre-file/per-env-member mutation and restore intents; deterministic faults before/during/after each step; close/reopen recovery classifies prior/desired/partial/foreign states | Apply terminal, notification-only, file active, environment active, mixed recovery, interrupted file/single-environment/mixed rollback, and committed lagging-ledger reconciliation covered by SetupRecoveryTests 266/266; Recovery+Journal 603/603; actual Apply producer→reopened Recovery consumer passes; notification ambiguity remains |
...
- The normal rollback producer is not yet implemented or independently
  ...
  recovery, and committed lagging-ledger reconciliation evidence is complete
  through Task 5d3b2b1. Notification ambiguity remains an explicit follow-up
  review item.
