...
| Transaction and rollback | #66 | In progress through Task 5d2 | `2d88eff..80004d9` | journal/store, exact artifact reuse, terminal/active Apply recovery, apply/compensation, and lock-lifetime evidence above | Tasks through 5d2 and Lock A-C PASS/APPROVED | rollback recovery remains |
...
| Crash recovery | Pre-file/per-env-member mutation and restore intents; deterministic faults before/during/after each step; close/reopen recovery classifies prior/desired/partial/foreign states | Apply terminal, notification-only, file active, environment active, and mixed recovery covered by SetupRecoveryTests 168/168; actual Apply producer→reopened Recovery consumer passes; rollback recovery pending |
