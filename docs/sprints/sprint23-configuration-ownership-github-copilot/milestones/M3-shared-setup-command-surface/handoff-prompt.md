...
7. docs/decisions.md, especially D058
...
complete. At this historical handoff, Issue #67 T7 was to wire `Program.cs` to
the production dispatcher and prove the real aggregate/three-partition
producer-consumer path plus direct-CLI and repository-wrapper `setup status`
`status_ready`/exit-0 byte parity. That successful-status subprocess assertion
is superseded by the dated correction below. T8 release packaging/operator
documentation and T9 final joint reviews remain open. Resume from the Issue #67
plan, not from the historical `399f441` repair sequence above.

## Dated evidence correction (2026-07-16)

This historical handoff is retained for its task sequencing and review record.
Its direct-CLI/wrapper `status_ready`/exit-0 assertion must not be used as
current evidence: a child-process `LOCALAPPDATA` value does not isolate the
production root, because `SystemSetupPlatform` resolves it through
`Environment.GetFolderPath` and has no setup environment override. Successful
status/storage and real #66→#67 behavior are instead proven by executable
in-process production composition with an injected trusted platform. Direct
CLI/wrapper subprocess evidence is limited to pre-dispatch invalid-input
byte/exit parity.
