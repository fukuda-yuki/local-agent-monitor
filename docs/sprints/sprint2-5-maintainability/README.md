...
| [M2: ConfigCli responsibility split](milestones/M2-configcli-responsibility-split/task.md) | 未着手 | `Program.cs` を entry point と責務別ファイルへ分割し、既存 CLI contract を維持する |
| [M3: Output helper consolidation](milestones/M3-output-helper-consolidation/task.md) | 未着手 | CSV / JSON 共通処理を集約し、出力互換性を確認する |
