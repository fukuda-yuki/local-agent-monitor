...
M2 で literal / regex / field predicate のレベルまで判定条件を定義してから M3 実装に進む。
M3 の最小実装は、M2 で確定した rule だけを実装対象とする。
...
| `DIAG-CONTENT-ERROR-MESSAGE-V1` | raw span / event content | M2 で定義する error field / status / message pattern に一致する | `F-ERROR` / `AP-ERROR-BLIND`、severity `major`、target `workflow` |
| `DIAG-CONTENT-SENSITIVE-LEAK-V1` | raw prompt / response / tool arguments / tool results | M2 で定義する credential / secret / Base64 header / identity pattern に一致する | `F-DATA` / `AP-RAW-CONTENT`、severity `blocking`、target `workflow` |
...
M2 が完了するまで、content-aware rule set は確定済みとはみなさない。
