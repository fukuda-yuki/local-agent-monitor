...
## Trace source-version interpretation corrections

Trace-scoped `source_trace_version_observations` are immutable capture facts.
Issue #154 adds append-only interpretation revisions and exact heads rather
than updating those rows. Only the internal `SourceCompatibilityReconciler`
may append a revision: decoder replay can recover a version from the same
retained bytes, and registry revision can recognise an already retained exact
token. A separate later resolved observation does not neutralize an older
missing observation.

These additions are Monitor schema v11 over the exact supported v10
predecessor. The v11 migration replaces the trace-version child cascade with
restrict semantics and installs base/ledger immutability triggers before
declaring v11. A declared v11 database with a partial authority, or an older
declaration with colliding v11-owned objects, fails before mutation.

Current effective aggregation, operation fingerprints, atomic compatibility
revision/invalidation and backup rules are owned by
