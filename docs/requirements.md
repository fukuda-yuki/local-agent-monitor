...
- Historical efficiency analysis。Issue #72 の exact canonical `historical-evidence.repository-safe.v1` bytes だけを入力にし、token volume、context growth、cache inefficiency、retry overhead、exact repeated tool-call volume、permission wait、subagent fan-out、duration outlier、opaque model mix を versioned deterministic rule で評価する。missing / unknown は zero に変換せず、exact tool-failure status がない v1 では tool failure overhead を `unavailable` とする。quality evidence は強度判定を downgrade できるが、効率観測から quality improvement / effect / monetary cost を主張しない。結果は exact evidence refs、category coverage、quality availability、fixed mitigation を持つ canonical repository-safe receipt とし、Issue #75 が formula を再計算せず消費する。HTTP、UI、DB/migration、import、AI narrative、proposal/effect/apply は含めない。
