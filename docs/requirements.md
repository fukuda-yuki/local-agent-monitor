...
- Skill invocation snapshot foundation（Issues #119/#157/#158）。凍結済み`POST /api/session-ingest/v1/events`でsupported eventは`skill.started | skill.completed`、`skill.invoked`は既存unsupported-event処理とし、route、header/body version、envelope/event shape、enum、1 MiB/100-event limits、validation/status/error bytes、`204`、workspace bytesを変更しないこと。additive raw-default-only `POST /api/session-ingest/v2/events`、`skill_invocation_snapshot:1`、historical body/path read、explicit current-file read、discovery、backup/sanitized compositionの受理済み方向は[Skill Invocation Snapshot Interface](specifications/interfaces/skill-invocation-snapshot.md)のみを正本とすること。current-valid claimは#154の単一authorityを再利用し、cross-arm linkはproducer trace IDとspan ID双方のexact一致だけを許し、name/path/time/cardinalityなどで結合しないこと。v2 parser/writer、component migration、raw-local route、host registrationは、exact outer wire/mapping、status/error/media/405 bytes、producer schema/fingerprint/registry seed、equality receipt/content byte domains、multi-fault/nullability/name/path mapping、success/discovery literals、historical-to-discovery handle identity proofがcanonical contractで全て確定するまで`BLOCKED_DECISION`とし、production `READY`と扱わないこと。snapshot namespaceはsanitized export/importから完全除外しempty carrierも作らず、raw snapshotはinvalid claimを復活させず、fallback/compatibility/dual pathを追加せず、Issue #152は未解決のままとすること。
