...
- Local Monitor v1。loopback-only の単一ユーザー向け human UI として、AI に依存しない Repository / Session 観測・調査 core を提供すること。利用者は Session Explorer でセッションを探し、必要に応じて Repository で絞り込み、Session detail で instruction、Token、cache read / new input / cache write、Skill、Tool、Sub-agent、error / retry、階層、timing、exact technical evidence を確認できること。情報が欠落・未対応・未観測・capture gap・certification pending・expired の場合はその状態を表示し、zero や近接関係から推測しないこと。Session summary、timeline、node、raw content は一つのcoherent snapshotとSummaryが発行したrevisionを使用し、別revisionのfactを混在させないこと。詳細な product boundary は [Local Monitor v1 Product Definition](specifications/interfaces/local-monitor-v1-product-definition.md)、route / page / state / vocabulary は [Local Monitor v1 IA Specification](specifications/interfaces/local-monitor-v1-ia.md)、責務の単一 authority は [Local Monitor v1 Contract Index](specifications/interfaces/local-monitor-v1-contract-index.md) を正本とすること。
- Local Repository catalog / Session assignment。Repository identity は exact canonical GitHub locator、Git が返す common management directory から生成した domain-separated opaque locator、opaque ID に限定し、name、prompt、time、cardinality、source label から推測しないこと。`local_repository_catalog:1` は immutable locator/head/history、exact observation provenance、manual override、assignment revision、durable idempotency receipt、virtual scope、raw-expiry 後も残る catalog-owned metadata、runtime backup vector を一つの authority として所有すること。完全な DC156-01–19 契約は [Local Repository Catalog and Session Assignment](specifications/interfaces/local-repository-catalog.md) と [DC156-12–19 executable closure](specifications/interfaces/local-repository-catalog-executable.md) により `READY_FOR_IMPLEMENTATION` である。#156 は catalog/assignment core と `ILocalRepositoryScopeSnapshotService` を所有し、#161 は direct Session/Repository archive facts だけを同じ service に供給し、#156 だけが exact assignment と facts を検証して effective archive eligibility/reason を合成し、#134 だけが `GET /api/local-monitor/v1/repositories` を map/serialize すること。Issue #152 の unknown attribute-key drift は未解決のままとすること。
- Copilot CLI Repository locator acquisition。exact native Session ID に対応する bounded `workspace.yaml` の exact `id` と明示的 `cwd` は locator そのものではなく、同じ trusted local user の checkout に対する Git authority の場所としてのみ使用してよい。remote URL から accepted canonical GitHub locator が一つだけ得られる場合はその locator を使用する。accepted GitHub locator がない場合は Git が返す absolute common management directory の domain-separated opaque identity を使用し、同じ Git repository の worktree はまとめ、別 clone は分ける。metadata mismatch、複数の conflicting GitHub remote、または Git authority 不在は unassigned とすること。path は保存せず、`events.jsonl`、directory scan、tool path、branch、commit は使用しないこと。
- Local Monitor v1 の primary route は `/`（Session Explorer）、`/repositories/{repositoryId}/sessions`、`/sessions`、`/sessions/unassigned`、`/sessions/{sessionId}`、`/repositories/{repositoryId}/comparisons/{comparisonId}` とすること。Repository scope navigation を primary pages に置き、狭い画面では折りたたむこと。generic aggregate/KPI dashboard、trace-list master-detail preview、tree/waterfall の別 primary view を置かず、breadcrumb と contextual search、Unified Settings modal を使用すること。旧 `/traces` list は Session Explorer 実装時に退役し、`/traces/{traceId}` と既存 raw record/span/event route は低レベル technical evidence として保持すること。
...
- Local Monitor v1 archive / Settings。Session / Repository archive は reversible local metadata であり、delete、retention、pin ではないこと。default list / Compare / Repository-range AI から除外し、direct Session access と explicit single-Session AI は維持すること。Repository archive は Session に cascade せず、新規 ingest で自動 restore しないこと。Issue #161/D082 は [Local Archive v1](specifications/interfaces/local-archive.md) を単一の executable authority とし、`local_archive:1` の current/history、mutation/read/list、direct-fact contributor implementation、exact raw-default `GET /api/local-monitor/v1/archive` / `POST /api/local-monitor/v1/archive-actions` / `GET /api/local-monitor/v1/archived-items` wire、runtime-backup validation を所有すること。#161 は eligibility/reason を事前合成せず、#156/D081 だけが exact assignment と complete direct facts を検証・合成すること。archive route/application/contributor は `--sanitized-only` human composition から、archive state/history/carrier は sanitized evidence から欠落させ、Issue #161 は UI を追加しないこと。receiver、AI、Repository、archive、storage/backup、diagnostics は一つの Unified Settings modal から開き、複雑または destructive な操作だけ focused detail flow を使うこと。
...
- SourceCompatibility correction / Skill projection（Issue #154）。trace version のbase observationはimmutableとし、`missing + resolved = missing`を維持する。同一retained inputのdecoder revisionまたは保存済みexact tokenのregistry revisionだけを、単一の内部`SourceCompatibilityReconciler`がappend-only interpretation revisionとして追加できること。意味が変わるcorrection、current OTel Skill invalidation、compatibility revision、exact ordered input frontier、generation、queue、idempotency receiptを一つのSQLite transactionにし、workerは全frontier itemのRetention operation leaseとrevision/frontier/projector/queue fenceを再確認してからだけpublishすること。単一Skill claim read authorityはarm別predicateを適用し、OTel armだけをcurrent resolved trace revision/generationへ結合し、SDK Session/Event armはtrace/spanなしでもexact local Session/Event/source identityと完全なapplication/adapter/normalization/payload schema fingerprint tupleをcurrent registryで評価できること。両armのmergeはproducerが明示したtrace IDとspan IDの双方がexact一致する場合だけとし、同一Sessionでexact mergeできないpositive observationは加算せずcountを`null`、stateを`certification_pending`とすること。raw snapshotはinvalid claimを復活させないこと。旧pre-release Skill rowはcopy/backfill/dual readerなしで捨て、独立`skill_projection:1`だけを作成し、無関係なSession/raw/Retention dataは保持すること。詳細は[Source Compatibility Reconciliation](specifications/layers/source-compatibility-reconciliation.md)と[Skill Projection](specifications/layers/skill-projection.md)を正本とする。Issue #152のunknown attribute-key driftは未解決のままとする。
- Skill invocation snapshot foundation（Issues #119/#157/#158）。凍結済みv1とD083のwire/schema/receipt/store/read契約は維持し、D086だけがproducer topologyをLocal Monitor所有raw-analysis Sessionへ置換し、D089だけがcurrent releaseのlive tuple scopeを置換し、D090がcurrent releaseのchronology/acceptance sequencingを訂正すること。外部CLI/VS Code Sessionはunavailable/unobservedのままenumerate/resume/history readせず、already-implemented current releaseはexact bundled `1.0.75` / protocol `3`のsame-client post-implementation T0bとdeterministic r0002/identity/admission coverage、candidate-bound platform evidence、pinned full validation、fresh independent reviewをrelease acceptanceとし、future registry revisionまたはproducer/startup implementationでは実装前T0bをmandatory gateとすること。そのT0bでVersion/Protocol/SessionStartと、prompt-free inventory-probe Sessionから`DisabledSkills`を固定した別execution Sessionのretained-root-only inventory/invocation/task completionを証明すること。r0002 deterministically admits exactly `1.0.65` and `1.0.75`（ともにprotocol `3`）を維持し、`1.0.65`はdeterministic compatibility/admission coverageであってmandatory live laneではないこと。probe Sessionは既存のexact source-qualified custom raw-analysis tool entriesだけを許可しpromptを送らないこと。retained rootsを構成したexecution Sessionだけが同じcustom entriesにexact `builtin:skill`と`builtin:task_complete`を加えたunionを許可し、wildcard、その他のbuilt-in、MCP、plugin、ambient instruction/config discoveryを許可せず、Skill自身の`allowed-tools` metadataもallowlistを拡張できないこと。両Sessionはcallbackをcreation前に登録し、execution Sessionだけがproducerとなること。通常producerはrequested promptを`AgentMode.Autopilot`で送信し任意のretained Skill invocationを強制せず、exact typed task-complete eventだけをterminal boundaryとすること。deterministic T0bだけはSDK commands APIでexact admitted enabled/user-invocable retained-skill commandを解決し、`executionSession.Rpc.Commands.InvokeAsync`で呼び出し、prompt-producing resultのexact returned promptを`AgentMode.Autopilot`で送信して、exact matching typed retained `SkillInvoked`の後にexact typed task-complete eventが続くことを証明すること。explicit `--skill-discovery-directory` rootsだけをSkill provenanceとし、collision、missing/unverifiable path、inventory drift、disable不能をprompt前にrejectすること。全retained inventory pathをprompt前にnative retained-root opener/leaseで証明し、各invocation pathをinvoked時に再証明してSDK文字列を信頼しないこと。callbackは0..64件かつcomplete v2 UTF-8 body合計8,388,608 bytesまでprocess memoryに保持すること。zero invocationはv2/v1 writeを一切行わず、positive invocationだけがexact completion後に同期・非durableな**owned-session post-completion buffer/import**としてv2順次送信、one-event v1 SessionStart、one-event v1 task-completeを実行すること。失敗はretryせずvalid committed prefixだけを残して旧current generationを維持し、全成功とSession dispose後だけcandidateをpublication-orderでcurrentにすること。zero rootsはservice/POST不在のouter `404`、rootsあり/current generationなしは既存exact `503 skill_current_file_discovery_unavailable`、zero invocationはsnapshot/receipt/Session eventなしだがsuccessful generation publicationを許すこと。wire/schema/receipt/storage/option/#154 authority、foreign producer、durable queueまたはimporter receipt/startup recovery/backfill/fallbackは追加しないこと。詳細は[Skill Invocation Snapshot Interface](specifications/interfaces/skill-invocation-snapshot.md)を正本とし、未解決作業と実装状況はGitHub Issuesを正本とすること。
- Codex App discovery（Issue #92）。承認済みの結論は Desktop production integration に対する `NO-GO` とする。Desktop package `26.715.10079.0` と独立して起動した Codex CLI/app-server `0.145.0` の組合せで、content-disabled、per-command override、disposable loopback receiver による standalone app-server の OTLP JSON trace、source trace/span/parent ID、timing、source version detector、構成済み content gate の形だけを確認した。`0.145.0` は public CLI version command の結果であり、観測した `service.version` key の値ではない。Desktop-bundled producer は存在を確認できたが WindowsApps access control により terminal から直接実行できず、standalone producer を代替証拠にしてはならない。安全な process-tree 診断は process ID、parent ID、executable path だけを memory 内で読み、package-root `codex.exe` と package-root OS parent の関係を fixed boolean で観測した。command line は読まず、child role/app-server identity、Desktop-owned OTel execution、App Session/window、merge authority を証明しない。先行する command-line-reading 診断は無効化して採用証跡から除外し、その値を保持していない。live probe の verbatim command、exact harness、raw output は保持されておらず、fixture の count/key/shape は独立再生成できない sanitized attestation とする。これは native thread/turn と OTel の結合、並行 window/thread、restart/resume、logs/metrics、または semantic field の完全性を証明しない。protocol native thread と OTel は観測上 `unbound` だが、turn は未実行なので `unverified` とする。generic `thread.id`、process、repository、workspace、cwd、timestamp、arrival order による結合を禁止する。v1 manifest は standalone-only availability を表現できないため、Codex App manifest は独立に確認した source version detector だけを `available` とし、trace を含む Desktop-specific capability は `unknown` のままにする。Issue #92 は adapter、Setup、Doctor、UI、private-state read、content-enabled capture を実装しない。Issue #93 の production adapter、Setup、Doctor、UI、manifest trace promotion、release validation は、別途承認された discovery retry と prerequisite configuration specification が Desktop-owned execution、retained repository-safe replay harness、exact configuration/detection、安全な log-export policy、source identity/parentage、exact または明示 unbound の native correlation を確立するまで開始しない。一部の既存 Codex App log-export / Langfuse / Collector sample は log exporter を有効にし、`log_user_prompt=false` でも `rust-v0.145.0` の tool-result logs が arguments、output、error を含み、content や path を漏らし得るため repository-safe とみなさない。Issue #92 は既存 production sample を変更せず、safe default または明示的な non-content mechanism を定義・検証する別途承認済み prerequisite specification と content-bearing profile の個別 authorization を high-severity blocker とする。
...
- Historical Analysis。Local Monitor v1 は `/api/historical-analysis/v1/*` の frozen machine contract だけを保持し、standalone `/historical-analysis` human page とその asset は #164 integration で empty 404 として退役する。machine API は #72 preview、独立した #73 instruction / #74 efficiency、#53 safe-token resolution、全 frozen state、repository-safe response、loopback/Host/same-origin/CSRF/no-store/strict JSON 境界を変更しない。current host が `--sanitized-only` の場合、preview は `selection.sanitized_only=true` の exact request だけを受け付け、`false` は #72 owner を開く前に `400 invalid_historical_analysis_request` で拒否し、server が `true` へ書き換えない。redirect、alias、compatibility page、browser state は追加しない。
...
- Canvas Improvement proposals。Canvas Improve は、exact-bound で current status が正確に `completed|failed`、completeness が `full`、かつ exact Session scope に non-null Session v14 terminal fact が1件以上ある Session の evidence を利用者が確認して作る local-runtime proposal lifecycle を提供する。proposal の terminal lookup は Session-scoped とし、Run/Trace は既存 evidence DTO が持つ関係だけを検証する。詳細分析は既存 `session.send()` + bounded Canvas actions の dispatch を維持し、Local Monitor raw analysis runner に置換しない。proposal は `candidate`、`recommended`、`verified` の固定 lifecycle とし、Candidate は citeable evidence、Recommended は少なくとも2つの distinct exact-bound Session の evidence と利用者による明示 promotion を要する。`POST` は insert と同一 transaction で current eligibility/evidence を再検証し、失敗を既存 `evidence_not_exact_bound` とする。promotion は lifecycle 更新と同一 transaction で全 contributing Session/evidence と2-Session条件を再検証し、失敗を既存 `insufficient_recommendation_evidence` とする。Verified は Issue #56 の comparison verdict に限定する。後の Session 再分類は proposal と receipt の履歴を変更しないが、current action/use は同じ eligibility を再検証する。proposal は target kind/opaque label、sanitized rationale、expected effect、risk、opaque evidence references を local runtime に保存してよいが、raw prompt / response、tool args/results、PII、credential、token、local sensitive path、source fragment を保存・action/log/prompt/repository-safe outputへ送出してはならない。proposal creation / promotion は loopback、same-origin、CSRF を要する明示的なユーザー操作であり、自動生成・自動 promotion・file/config/Skill/Agent/Instruction の自動変更を行わない。direct apply、diff、snapshot、rollback、git操作は Issue #55 のみが扱う。
...
- Canvas effect comparison。Issue #56 は、exact Session / Run / trace と immutable objective evaluation receipt、Issue #54 proposal revision、active Issue #55 application receipt に基づき、利用者が明示確定した pre/post cohort の効果を quality-first で比較する。objective receipt は pass/fail、normal/severe、evaluator ID/version、criterion、case key、exact evidence refs を持ち、repository/timestamp proximity や normalized measurement の unlinked `success_status` を evidence に昇格させない。objective creation/use と included Session は current に exact-bound・`completed|failed`・`full`・Session-scoped non-null fact を満たし、pre/post 各3件未満、missing/partial/conflicting evidence、rollback/stale application は `insufficient_evidence` とする。quality pass rate と severe regression を efficiency より先に判定し、quality 同等時だけ duration / total-token median の10%境界を使う。verdict は `improved` / `no_change` / `regressed` / `insufficient_evidence` の固定4値で単一総合 score を作らない。`improved` の effect receipt 保存と proposal `verified` 更新は同一 transaction とし、同 transaction 内で全 included Session の eligibility を再検証する。rollback またはいずれかの included Session の eligibility 喪失後は current effect projection を `invalidated` とする一方、objective/effect receipt・verdict・proposal 履歴は不変に保持する。cohort confirmation / comparison writes は loopback、same-origin、CSRF、no-store の明示操作であり、raw content、path/source/diff、automatic Verified、git 操作を含めない。
...
- GitHub Copilot app Canvas adapter。Local Ingestion Monitor の既存 monitor
  ...
  path、raw OTLP payload を返してはならない。Canvas Adapter M5 では拡張所有の loopback
  ヘルパーページ上に「Analyze selected trace with Copilot」UI トリガーを任意提供し、
  トリガー指示は選択した trace id・optional span id・focus・action 名のみを含み、
  monitor payload や raw / PII を埋め込まない（D029）。Canvas Diagnostic Surface では、拡張所有
  ...
  Canvas Repository Metadata では cross-repo 利用のため、`.github/extensions/otel-monitor-canvas/`
  ...
  repository-safe output 送出は追加しない。Canvas Analysis UX では既存の
...
### D089 Current release live-proof authority

Current r0002 deterministically admits exactly `1.0.65` and `1.0.75`, both at
protocol `3`; D087's exact two-entry registry is unchanged. For this release,
the mandatory T0b and final signed-in Windows live tuple is exactly bundled
`1.0.75` / protocol `3`. Deterministic compatibility and admission coverage for
`1.0.65` remains required but is not the mandatory live lane. This paragraph
supersedes the earlier D083/D086 requirement to live-prove every admitted tuple;
all runtime identity checks remain exact and fail closed.
Release acceptance requires the #158 owned-session importer, candidate-bound
Windows/Linux platform/live evidence, pinned full validation, and fresh
independent review.

### D090 Current release chronology correction

Durable history shows that D086/S3 authority and r0002/producer implementation
preceded the accepted signed-in live proof. The former claim that current-release
T0b preceded r0002 and producer implementation is retracted. For this already-
implemented release, the exact post-implementation bundled `1.0.75` / protocol
`3` same-client proof resolves that historical sequencing conflict together with
deterministic r0002/identity/admission coverage. Because production changed after
that proof, every later candidate requires fresh Windows/Linux evidence; pinned
full validation and fresh independent review remain release gates. Future registry revisions or
producer/startup implementations still require T0b before implementation. D090
changes chronology and acceptance sequencing only; every identity, topology,
fail-closed, wire, schema, HTTP, CLI, storage, D087, and D088 contract remains
unchanged.
