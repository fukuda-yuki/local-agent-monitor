...
- GitHub Copilot app Canvas adapter で Local Monitor UI を再実装すること、Issue #51 で明示した Session ingest / workspace / raw-content interfaces 以外の telemetry input / raw endpoint / schema / API field を追加すること、raw prompt / response body、tool arguments / results、PII、credential、token、local sensitive path を Copilot actions へ返すこと。Issue #51 の Session UI は Issue #52 の current-screen capture と承認済み four-tab prototype が完了するまで着手しない。direct apply、Compare、Agent graph、Issue #57 の physical cleanup / pin / delete-now は含めない。
