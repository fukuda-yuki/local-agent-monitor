...
- Historical source import policy。Tier A の product-owned versioned artifact と exact fixture/fingerprint に結合した Tier B producer history だけを候補 source とし、private / heuristic Tier C store は対象外とする。current GitHub Copilot CLI / Claude Code profile の supported application/format sets と active allowlists は空で、明示 opt-in detector は content を読まず zero candidates を返す。#77 / #78 による exact fixture-bound promotion は D056 ceiling 内の technical compatibility revision とし、#79 は sparse 1–10 allowlisted leaves と field provenance の exact parity、no synthesis、partial / `historical_summary_only`、exact merge、fixture marker rejection を検証する。metadata-only は retention item を作らず、将来の consented content だけを #89 / #90 の契約へ渡す（D066）。本項は policy/contract のみで adapter、importer、background scan、migration、UI を実装しない。
- Alert-rule engine foundation。versioned source-neutral snapshot / capability と compiled rule registry を deterministic に評価し、missing / unknown / unavailable evidence を zero と推測せず bounded suppression にする。accepted match は exact evidence、config/input/evaluation hashes を持つ immutable canonical receipt として component-owned `alert_engine` schema v1 に保存し、sensitive comparable label は private keyed HMAC token にする（D067）。#81 / #82 の concrete rules、#83 lifecycle、#85 UI/export はこの frozen foundation の consumers であり、receipt bytes や engine-owned tables を再定義しない。canonical receipt を別 component が消費する場合は、#80 所有の strict v1 consumer validator を使用し、unknown / duplicate field、非 canonical byte、version/profile/invariant 違反と receipt 内 field から再計算できる `alert_id` の不一致を固定 no-leak error で拒否すること。consumer boundary は 8 MiB と receipt v1 の固定 JSON depth に制限し、返却 projection は alert/session/optional trace/source surface/last-observed identity のみとする。この上限と consumer-only semantic gate は既存 producer/serializer/store admission や bytes を変更せず、超過または semantic-invalid receipt を downstream が truncate してはならない。receipt 単体から再計算できない evaluation/config/input identity は canonical lowercase hash shape のみを検証し、origin/signature/authorization/provenance を主張しない。
- Sanitized evidence export。公開 HTTP / CLI control は `sanitized-export-control.v1` の schema version、UTC `created_at`、selection のみとし、caller snapshot / record bytes / safety marker / output path を受け取らないこと。明示された既存 Local Monitor database から、一つの read-only SQLite connection と deferred transaction で monitor v7、Session v13、optional #59 / #80 schema を anchor し、bounded descriptor と byte-length gate の後に selected exact-ID carrier だけを読むこと。`raw_records`、`session_event_content`、raw analysis/content store は query せず、#59 / #80 の公開 consumer validator と exact canonical bytes を再利用する。#58 safe metadata projection、#59 handoff、#80 receipt 以外の future carrier を受理せず、snapshot ID は selected inventory / exact hashes / capabilities / versions の length-framed canonical identity とする。preview / create は同一 snapshot を一度だけ capture し、scanner / producer / archive failure では partial artifact や success bytes を返さない。`result` は独立して strict archive / checksum / canonical carrier を検証するが provenance / origin attestation を主張しない。import、raw replay、backup/restore、upload、signing は #86 / #87 / #88 または別 issue の責務とする。
...
- Cross-surface first-trace closeout。Issue #105 では `github-copilot-vscode`、`github-copilot-cli`、`github-copilot-app-sdk`、`claude-code` を一つの固定 source registry として Config CLI、Release ZIP、Local Monitor の `/diagnostics` Doctor section から選択できること。CLI JSON、human summary、UI proxy は同じ persisted `FirstTraceEnvelope` と埋め込まれた未変更の `doctor.v1` result を投影し、UI は source 固有 state、優先順位、missing-fact fallback、latest/timestamp heuristic を追加しないこと。UI proxy は additive な `/api/doctor/ui/v1` route 群とし、既存5つの `/api/doctor` routeを変更しない。Doctor result の opaque evidence reference から exact trace、local Session、source diagnostic observation への navigation は、server が検証済み target kind、opaque ID、固定 same-origin relative href を別の sanitized projection として返す場合だけ許可する。Local Monitor の情報設計は D042 の7画面・2項目ナビを維持し、Doctor、exact Session summary、source diagnostic target は新規画面ではなく `/diagnostics` 内へ表示すること。Windows x64 Release ZIP は source build、手動環境変数編集、.NET SDK/Runtimeを要求せず、同じ first-trace command surface と rollback/uninstall 後の整合した Doctor state を提供すること。
