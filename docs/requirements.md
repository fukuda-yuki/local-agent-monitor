...
- Skill invocation snapshot foundation（Issues #119/#157/#158）。凍結済み`POST /api/session-ingest/v1/events`でsupported eventは`skill.started | skill.completed`、`skill.invoked`は既存unsupported-event処理とし、route、header/body version、envelope/event shape、enum、1 MiB/100-event limits、validation/status/error bytes、`204`、workspace bytesを変更しないこと。D083で残る8 gate、すなわちexact normalized v2 wire/SDK mapping、status/media/method/error/precedence、payload schema/fingerprint/registry、equality receiptとselected identity/replay、payload/content bytesとsole raw owner、total classification/bounds/nullability/claim validity、literal success/fresh serialization、historical-to-discovery/native current-file readは全てProduct Owner決定済みであり、詳細とexact artifact identity/hashは[Skill Invocation Snapshot Interface](specifications/interfaces/skill-invocation-snapshot.md)のみを正本とすること。additive raw-default-only `POST /api/session-ingest/v2/events`はSDK 1.0.4のrequired `name,path,content`とoptional `allowedTools,description,pluginName,pluginVersion,source,trigger`だけを受け、`model`を持たず、application-bundled CLI `1.0.65`、protocol `3`、adapter `copilot-sdk-dotnet-1.0.4+cao-skill-v2.1`の同一client runtime capabilityだけから一つのHTTP bridgeを通ること。success responseはvalidated semantic factsから毎回freshに生成しresponse bytesを保存・replayせず、current-file rootはrepeatable CLI argv（Windowsでは同じargvをTask Scheduler actionへ格納）以外のcarrierを持たないこと。Windows NTFS/ReFS gateとLinux 5.8+ `openat2`/ext4・xfs・btrfs gateは独立し、zero rootsではcurrent-file service/POSTを登録せず、explicit rootsを与えたuncertified platformはhost startupをfail closedし、absolute-path/.NET fallbackを持たないこと。canonical promotion後に#117/#119/#124/#156/#157/#158/#161のlive Issue reconciliation/readbackが成功すれば、#119のstrict bounded single-pass parserとimmutable capability-bearing typed handoffはnonregisteredで独立実装可能とすること。#158 runtimeは#124 Session 14、D081の#156 carrier/composition、D082の#161 direct archive implementation/backup/restore、別landのRetention pinned-read implementation/tests、nonregistered #119 handoffが全てintegratedかつgreenになった後だけ開始し、host activation/route registration/releaseはfocused/platform/signed-in live/full-validation/review gateまで`NO-GO`とすること。current-valid claimは#154の単一authorityを再利用し、cross-arm linkはproducer trace IDとspan ID双方のexact一致だけを許し、name/path/time/cardinalityなどで結合しないこと。snapshot namespaceはsanitized export/importから完全除外しempty carrierも作らず、raw snapshotはinvalid claimを復活させず、migration adoption/backfill、fallback、compatibility alias/writer、dual reader/pathを追加せず、Issue #152は未解決のままとすること。
