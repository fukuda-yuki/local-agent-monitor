...
Sprint3 以降では、trace content を含む観測データから診断候補、改善候補、自動採用判断、自動改善実装へ接続する後続基盤を扱う。
ただし、実 repository 修正を伴う自動改善実装は Sprint3 の既定スコープには含めず、Sprint4 以降の候補として安全境界を定義してから扱う。
...
Sprint3 以降では、明示的に仕様化した範囲に限り、診断候補生成、改善候補生成、自動採用判断、自動改善実装を後続スコープとして扱う。
...
Sprint3 以降では、改善候補の自動決定と改善案の自動採用を扱う。
実 repository file の自動修正、patch / diff の生成、commit / push / pull request 作成は、Sprint3 では扱わず、Sprint4 以降で安全境界を定義してから扱う。
...
Sprint3 では、trace からの自動診断候補生成に加え、content-aware evidence、改善候補生成、自動採用判断の基盤を扱う。
