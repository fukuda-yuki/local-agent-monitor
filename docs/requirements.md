...
- Sanitized evidence import。Issue #86 は #85 の frozen `sanitized-evidence-bundle.v1` と #58/#59/#80 carrier だけを strict inspector 経由で受け取り、archive/profile/schema/checksum/canonical producer/scanner/size/path/duplicate/symlink/compression を fail closed で検証する。preview は source versions/date/count/capability/completeness、固定 migration chain/hash、exact new/duplicate/conflict、missing/external graph refs、expected record/graph/history/retention impact を示し、その digest と exact archive bytes を commit に結合する。同一 `(record_type, record_id)` + exact bytes は skip、同一 ID + different bytes は whole-import conflict とし、heuristic merge/overwrite はしない。commit は `sanitized_import` component v1 の record/origin/exact graph/history を一 transaction で all-or-nothing 保存し、再実行は同一 import receipt を返す。Session/monitor/raw/alert owner tables と retention catalog は変更せず、imported sanitized output は raw item ではなく Issue #90 target は `retention_target_not_applicable` とする。loopback API/UI と CLI は same-origin/Host/CSRF/no-store/bounded inert-text 境界を持つ。raw OTLP、content、backup restore、unknown/future profile、#72/#83/#73/#74/#84 carrier、silent conflict、embedded content execution は v1 対象外とする（D070）。
