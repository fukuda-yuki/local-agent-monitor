...
- Local Repository catalog / Session assignment。Repository identity は exact canonical GitHub locator と opaque ID に限定し、name、path、CWD、prompt、time、cardinality、source label から推測しないこと。`local_repository_catalog:1` は immutable locator/head/history、exact observation provenance、manual override、assignment revision、durable idempotency receipt、virtual scope、raw-expiry 後も残る catalog-owned metadata、runtime backup vector を一つの authority として所有すること。#156 は catalog/assignment core と `ILocalRepositoryScopeSnapshotService` を所有し、#161 は同じ service に archive eligibility を合成し、#134 だけが `GET /api/local-monitor/v1/repositories` を map/serialize すること。現時点で production 実装可能なのは exact locator parser/fingerprint だけであり、schema、admission/reconciliation、mutation/read routes、scope composition、backup registration は [Local Repository Catalog and Session Assignment](specifications/interfaces/local-repository-catalog.md) の全 blocker が解消されるまで実装しないこと。Issue #152 の unknown attribute-key drift は未解決のままとすること。
