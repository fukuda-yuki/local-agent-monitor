...
- Phase 1 baseline observability backend。
- Docker Desktop 上の self-host 構成を既定とする。
...
- masking、sampling、TLS、SSO、共有環境運用は初期範囲外。
...
- local PoC の既定 raw telemetry store。
...
- `artifacts/dashboard-input/measurements.json` があれば実データ由来の sanitized input として扱う。
...
- generated static dashboard artifact when intentionally published
...
The Aspire AppHost is retained only as Phase 0 background and build coverage.
...
Current default is local PoC plus optional private GitHub Pages dashboard.
Production deployment is not decided.
