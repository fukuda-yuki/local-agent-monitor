...
> `LOCALAPPDATA` value does not isolate production setup storage because the
> production platform resolves the root with `Environment.GetFolderPath` and
> has no setup environment override. The PATH-stripped packaged-wrapper check
> therefore proves only that the self-contained published executable is
> selected. Its current subprocess evidence is pre-dispatch invalid-input
> byte/exit behavior; successful status/storage and real #66→#67 behavior are
> proven by executable in-process production composition with an injected
> trusted platform. This correction supersedes any historical successful-status
> isolation reading of this T8 card.
...
- [ ] **Step 2: Write the failing packaging tests** in
  ...
  at its packaged path; a PATH-stripped packaged-wrapper invalid invocation
  (for example, `setup apply` without `--change-set`) has the fixed
  pre-dispatch stdout/stderr bytes and exit code. No `dotnet` on PATH is
  required for that release-mode run (assert via an environment that hides the
  SDK, following the existing self-contained test technique in this file).
  This is self-contained executable-selection evidence, not successful
  `status_ready` or storage-isolation evidence.
