...
| **[DOC-5A](#DOC-5A)** | Natural Japanese | All doc files (`README.md`, `user-guide*.md`) | Medium | Fixed | `humanizer-ja` 観点での「〜することができます」「〜となります」「〜について説明します」の過多 |
| **[DOC-5B](#DOC-5B)** | Natural Japanese | `README.md`, `docs/user-guide.md` | Medium | Fixed | 不自然な直訳英単語・名詞句（`reversible setup`, `canonical carrier`, `telemetry evidence`）の未翻訳露出 |
| **[DOC-5C](#DOC-5C)** | Natural Japanese | `docs/getting-started.md` vs `README.md` | Medium | Fixed | ファイル間・ファイル内での文体（敬体「です・ます」と常体「だ・である」）の不統一と過剰な受動態 |
