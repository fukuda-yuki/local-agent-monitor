...
このフェーズは評価支援までを対象とし、改善案の自動生成、自動実装、勝敗の自動決定は対象外とする。

### 5.2 measurement schema

研究用集計出力は CSV / JSON の両対応を想定し、少なくとも以下の列を持つ。
...
候補は UI export、API、DB 直接参照、ClickHouse 参照とし、まず API または export を優先する。
DB 直接参照と ClickHouse 参照は、API / export で必要なデータを取得できない場合の候補とする。
...
M23 以降でも、自動 repository 修正、自動 commit、自動 push、自動 pull request、自動勝敗決定は既定スコープ外とする。
