...
- Sprint10 design views（Flow Chart / Cache Explorer / timeline filter-sort / themed UI）は **public interface を追加しない**。新エンドポイント・新 API field・新 query parameter はなく、既存 `GET /api/monitor/traces/{traceId}/spans` の sanitized 出力を client-side で描画するのみ。raw-bearing route set も不変。
