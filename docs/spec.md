...
- Local Monitor v1 Session/node shared model discovery and per-run selection (#279, bounded #242 shared-capability tranche): raw-default `GET`/`POST /api/local-monitor/v1/ai/models` and required `model` on `POST /api/local-monitor/v1/ai/session-runs` and `POST /api/local-monitor/v1/ai/node-runs`. Discovery uses the pinned GitHub Copilot SDK `ListModelsAsync` through the same authenticated owned-client path as Session/node execution. It does not substitute `CopilotAnalysis:Models:*`, `DefaultModel`/`Model`, CLI help examples, or a hard-coded list. GET returns the process-local in-memory snapshot only; POST is the explicit list/refresh. The selected identifier is validated against the current usable discovered set, stored immutably on that run, included in that run's configuration hash, and passed as `SessionConfig.Model`. Page-local UI selection does not mutate User Secrets or restart the host. `auto`, silent fallback, automatic retries, and mismatch bypass are forbidden. Frozen Canvas/raw-analysis and Repository/Compare contracts are unchanged. Detailed human-UI and security rules are owned by [Local Monitor v1 IA](specifications/interfaces/local-monitor-v1-ia.md) and [security](specifications/interfaces/local-monitor-v1-security.md).
