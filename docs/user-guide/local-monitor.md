...
source/version・完全性と、exact に解決できた evidence リンクだけが表示されます。
一致結果は100件ずつ表示され、前へ/次へで全ページを移動できます。フィルターを
変更すると先頭ページへ戻ります。
見つからない、期限切れ、または identity が一致しない evidence は補完されず、
`missing` / `expired` / `unknown` としてリンクなしで表示されます。

Recurring は同じ rule/version、repository、workspace、source/version、UTC 観測日、
選択期間の組み合わせで集計し、2つ以上の異なる Session がある場合だけ
`supported` です。同じ Session の複数 receipt は recurring とみなしません。
Coverage に表示される suppression fact はアラートではありません。

取得上限に達して `snapshot_state: incomplete` になった場合、画面は取得範囲内の
結果として表示します。空でも全体の 0 件とは断定せず、概要の項目を「最新」とは
表示せず、Recurring も `incomplete_snapshot` のままです。

状態変更は詳細に表示された許可済み操作から行います。画面は表示中の revision を
送信し、競合した場合は上書きせず最新状態を再読み込みします。Alert Center が独自の
lifecycle 状態を持つことはありません。

評価は `POST /api/alert-center/v1/evaluations` への明示操作だけです。取り込み、起動、
ページ表示、GET では実行されません。canonical UUIDv7 Session と exact trace を指定し、
その trace の全 span が持つ `raw_record_id` ごとに #61 source observation の
surface/application version が一致し、trace の span count と persisted span 行数が
一致した場合だけ評価へ進みます。欠落、部分 projection、version 不明、混在は推測せず
拒否されます。既定の `raw-otlp` receiver は application version を持たないため、この
partition gate で拒否され、adapter version から version を補いません。現行 source
capability manifest は frozen rule が
必要とする capability を認可していないため、production 評価は receipt を生成せず、
成功した exact partition について10ルール分の `missing_required_capability` または
`source_not_applicable` を coverage として記録します。positive alert 表示は将来、exact source/version manifest と adapter が
認可された後に限って生成されます。

## First-trace Doctor

First-trace Doctor は、12 個の明示的な fact family を評価し、固定された 20 state から
診断結果を返す source-independent な core です。direct caller、Config CLI、Local Monitor
HTTP は同じ `doctor.v1` result を返します。Issue #102 の範囲には Doctor 用の proxy、
Razor / JavaScript / Canvas UI はありません。

### Fact snapshot を評価する

`doctor evaluate` は 1 個の strict な `DoctorFactSnapshot` JSON file を読みます。repository
同梱の synthetic fixture で CLI surface を確認する例です。
...
stdout は 1 個の canonical `doctor.v1` JSON です。この fixture は有効な non-ready 診断
...
window を開始し、状態を読み、明示的に cancel します。
...
`fact_snapshot.observations` は空でなければなりません。参照先 candidate の class / kind /
source / adapter / timestamp / expiry は caller が指定せず、store/service が既存の unexpired
candidate から trusted observation を構築します。Issue #102 は public な candidate command
や route を追加していません。GitHub Copilot / Claude Code の source-specific candidate
producer は #103 / #104、proxy / UI は #105 の範囲であり、現時点では live workflow として
未検証です。
...
setup の静的成功、`doctor evaluate` の処理成功、verification start による window 作成は、
いずれも最初の実 trace を保証しません。既存の real-source candidate を明示選択した complete
...
historical-only の trace 操作は無効です。exact に既存 Session へ結合された場合だけ、
その Session への navigation link を表示できます。
...
raw source、candidate/source-record key、confirmation / idempotency 値は、preview、結果、
...
この endpoint は opaque な ID、bounded な unknown 件数、reason code、次の操作だけを返し、
...
OTel が trace/span の identity・parentage・timing を、Hook が native session lifecycle と
explicit event identity を所有します。binding は identical native session ID、explicit
resume/handoff、または byte-equivalent trace context のいずれかだけで成立します。
repository、cwd、process、transcript path、timestamp の近さは binding 根拠ではありません。
parentage が欠落または曖昧な Claude hierarchy は `unresolved` のまま表示されます。

現行 Session DTO は完全な trace context を持たず `trace_id` のみを扱うため、
byte-equivalent trace-context binding は未完了です。共通 trace ID だけでは `exact_linked`
...
を確認しました。ただし次の live producer evidence は未取得です。

- interactive: authenticated TTY が無く、標準入出力が redirect されたため実行不可。
- `claude -p`: bounded run は exit `0` でしたが structural OTel / Hook telemetry を出力しませんでした。
- Agent SDK: supported package、runtime reference、認証情報が利用できず実行していません。

この結果は interactive / print / SDK の evidence を相互に代用しません。raw payload、PII、
credential、local path を記録しない named follow-up が必要です。詳細な記録は
...
Compare は、1 つの exact proposal revision と、その revision に結び付く active な
application receipt を対象に、利用者が明示確定した pre / post cohort を比較します。
対象の application は `applied` 済みで pending / rollback ではなく、登録 root 内の全 target が
記録済み post-apply SHA-256 と一致している必要があります。proposal revision の不一致、
recovery failure、rollback、post-hash stale のいずれでも比較結果は `insufficient_evidence`
です。path、source、diff、snapshot は Compare に表示・返却されません。

objective quality の receipt は、exact Session / Run / trace と、同じ scope に解決する
evidence refs を固定して保存します。receipt には pass / fail、normal / severe、evaluator ID /
version、criterion、case key、recorded time が含まれます。repository、時刻近接、prompt 類似、
normalized measurement の未紐付け `success_status` は evidence や Session の結合根拠にはなりません。

候補は非権威的な提示です。候補を含めることも verdict を作ることも自動では行われません。
...
選びます。included Session は exact-bound、terminal、`full` でなければならず、pre は
`applied_at` 以前に終了、post は `applied_at` 以後に開始している必要があります。境界をまたぐ
Session は含められません。case-key drill-down と summary は、同じ保存済み Session / evidence rows を
表示します。

判定には pre / post 各 3 Session 以上が必要です。各 included Session は human evaluation
（`expected` / `problem`）または immutable objective receipt の decisive quality evidence を少なくとも
1 つ必要とし、missing、partial、conflicting、out-of-scope の evidence は補完せず
`insufficient_evidence` とします。objective の severe fail が post に 1 つでもあれば
`regressed` です。

verdict は `improved`、`no_change`、`regressed`、`insufficient_evidence` の 4 種だけです。
まず severe と quality pass rate を比較し、quality が同等のときだけ duration と total-token の
median を比較します。10% ちょうどの改善は material、悪化は 10% より大きい場合だけ material です。
丸め表示は verdict を変えません。quality より efficiency を優先したり、単一 score にまとめたりは
しません。

`improved` の effect receipt 保存と proposal の `verified` 更新は 1 つの SQLite transaction です。
その後の rollback では receipt は履歴として残りますが `invalidated` となり、active improvement として
表示されません。Compare は loopback / same-origin / CSRF / no-store の明示操作です。Canvas action、
`session.send()`、log、repository-safe output に cohort / comparison payload を渡さず、automatic
