...
project-scoped extension で、モニター UI を再実装せず、既存の sanitized
`/api/monitor/*` API と `/health/ready` だけを読み取ります。

### Canvas-safe 姿勢

Canvas adapter を使う場合は、モニターを `--sanitized-only` で起動してください
（D030 参照）。このモードでは TraceDetail の sanitized tab（Summary / Timeline /
Flow Chart / Cache）は残りますが、raw section と full raw route が非表示になります。
`--sanitized-only` なしで起動した場合は、Canvas adapter が raw data を表示する
可能性があるため、推奨されません。

### 必要なもの

- GitHub Copilot app（Canvas extension runtime をサポートするバージョン）
- Local Ingestion Monitor を `--sanitized-only` で起動済み

### 使い方

1. モニターを `--sanitized-only` で起動します。

   ```powershell
   dotnet run --project src\CopilotAgentObservability.LocalMonitor -- --db data\monitor.db --url http://127.0.0.1:4320 --sanitized-only
   ```
...
4. ボタンを押すと、Copilot に sanitized analysis 指示が送信されます。Copilot は
...
| Action | 入力 | 出力 |
|---|---|---|
| `monitor_health` | なし | モニター到達性・readiness 状態・Canvas-safe 判定・診断メッセージ |
...
全ての action response は sanitized metadata のみを含みます。raw prompt / response body、
tool arguments / results、PII、credential、token、local sensitive path は
含まれません。
