...
| `http://127.0.0.1:4320/traces/{traceId}` | agent-execution view（Summary / Timeline / Flow Chart / Cache の tab 表示 + 下段に raw body inline） |
...
ページ上段は sanitized なデータを 4 つの tab で表示し、下段に raw body（既定表示）が
続きます。tab を切り替えても表示中の trace は変わりません。
