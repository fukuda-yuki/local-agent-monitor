...
| 画面 | 開き方 | 内容 |
|---|---|---|
| 概要 | `http://127.0.0.1:4320/` | トークン KPI（合計 / 実効入力換算 / キャッシュ読取率 / エラー trace）、期間トグル（今日 / 7日 / 30日）、モデル別内訳、キャッシュ効率、高コスト trace TOP5、時間帯別トークン、最近のトレース |
...
期間トグル、トークン合計と前期間比較、実効入力換算（キャッシュ読取 = 0.1x 換算）、
キャッシュ読取率、エラー trace 数（クリックでエラーのみの一覧へドリルダウン）を
表示します。下段にはモデル別トークン内訳、モデル別キャッシュ効率、高コスト trace
...
エラー / 遅延 / ツール利用 / エージェントの流れ）を選んで実行すると、captured raw
