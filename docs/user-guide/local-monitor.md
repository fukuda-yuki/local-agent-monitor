...
Local Monitor v1 を開くと、すべてのセッションが表示されます。検索してセッションを開くか、
左のリポジトリで対象を絞り込んで比較を作成します。
観測・調査・比較は AI なしで利用できます。GitHub Copilot SDK の AI 分析は任意です。

ヘッダーにはパンくず、受信状態、「設定」があります。受信状態と「設定」は同じ設定モーダルを開きます。
左側にリポジトリのナビゲーションがあります。狭い画面では「リポジトリ」を開いて切り替えます。

| 画面 | 開き方 | 内容 |
|---|---|---|
| ホーム | `http://127.0.0.1:4320/` | すべてのセッション。左側でリポジトリ・未設定へ切り替え |
...
| セッション詳細 | `/sessions/{sessionId}`（任意で `execution` / `node` / `analysis` / `settings`） | 活動、入力・出力・キャッシュ。「情報・指示」で指示や取得範囲を確認 |
...
初期表示は活動一覧です。「情報・指示」を開くと最初の指示、取得元、時刻、取得範囲、
技術情報を確認できます。開始時刻が無い場合は最終記録を表示します。
値のない項目は「なし」と表示し、記録された0件とは区別します。

上部に入力・出力トークンとキャッシュ率を表示します。「内訳・記録状態」から合計、
キャッシュ読み込み、新規入力、取得範囲を確認できます。ノードを選んでも上部の値はセッション全体です。
...
最新の実行が初めに展開されます。最上位が子を持つ単一のAgentなら最初の子ページも開きます。
ノードを選ぶとインスペクタで入出力を確認できます。時刻や関連する記録は「時刻・属性」で開きます。
...
別に取得できることがあります。記録状態の「未対応」は取得元の種類そのものでは
ありません。
...
「設定」→「AI設定」で GitHub Copilot の認証を確認できます。これは GitHub ホステッド経路の
認証確認であり、選択モデルが使えることや分析成功の証明ではありません。
セッション全体と個別項目の分析は、分析が設定されていれば GitHub の接続確認前でも開けます。
分析画面でモデル一覧を明示的に読み込み、使うモデルを選びます。
GitHub ホステッドの ID は認証済み SDK カタログから、BYOK の ID は既存の Copilot CLI
プロバイダー登録から取得します。設定の接続確認や `CopilotAnalysis:Model` の残存値、
ヘルプに載っている例は、利用可能なモデルの証明ではありません。
残っている設定モデルは、まだ何も選んでおらず、一覧に同じ ID があるときだけ初期選択になります。
現在の選択が一覧に無いときは選び直しが必要で、別のモデルへ自動では切り替えません。
分析画面を URL から直接開いた場合も、同じ一覧読み込みが走ります。
選んだモデルとその接続はその実行だけに記録され、共有 User Secrets の編集やモニター再起動は不要です。
モデルを選ぶと、送信先のサービスと、利用上限が GitHub ホステッド枠か BYOK プロバイダーかが表示されます。
SDK という実行経路の名前は、課金先の証明ではありません。
GitHub ホステッドのモデルは GitHub Copilot へ送られ、GitHub の枠を使います。
BYOK のモデルは設定済みプロバイダーへ送られ、そのプロバイダーの上限を使います。
セッション全体と個別項目の「再分析」は最大 600 秒待ちます。これは raw 解析の
`CopilotAnalysis:TimeoutSeconds`（既定 60）とは別です。分析に失敗した実行は残ります。
モデル一覧を更新して選び直し、新しい分析を開始してください。
SQLite ファイル全体や任意の SQL を渡す機能ではありません。
