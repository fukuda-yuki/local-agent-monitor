...
テレメトリにはプロンプトやツール呼び出しの内容など、機密性の高い情報が含まれることがあります。以下のルールを守ってください。
...
- 要約（マスキング済み）。
- 正規化した集計データセット。
- ダッシュボード用データセット（サニタイズ済み）。
- trace id / candidate id / evidence ref 等の参照 ID。
- 実データ由来の集計メトリクス。
- `user.id` / `user.email` を含む分類属性。ただし共有・公開前にアクセス制御を確認すること。
...
- システムプロンプトの全文。
- ツールの引数 / 結果の全文。
- 観測セッション由来のソースコード断片 / ファイル内容。
- 認証情報（credential、secret、token、API key、パスワード）。
- Base64 認証ヘッダー。
- 機密バンドルの内容。
- 機密バンドルのローカルパス。
