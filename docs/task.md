...
2026-05-19 に、Issue 発行テスト用の意図的な synthetic fixture として `tests/fixtures/gitleaks/intentional-finding.env` を追加した。
この fixture は実 credential ではなく、gitleaks `github-pat` rule を発火させるための一時的な検証データである。
ローカルで gitleaks `v8.30.1` の `dir` scan と、一時 git repository に commit したうえでの `git` scan の両方で finding が 1 件出ることを確認した。
GitHub Actions 上で Issue 作成を確認した後、この fixture は削除する。
...
- [ ] M10 の GitHub Actions Issue 発行確認後、`tests/fixtures/gitleaks/intentional-finding.env` を削除する: https://github.com/fukuda-yuki/copilot-agent-observability/issues/20
