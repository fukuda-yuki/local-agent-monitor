...
| Area | 状態 | 概要 |
| --- | --- | --- |
| GitHub Copilot Canvas Adapter PoC | **計画中 / M3 planned** | [docs/sprints/sprint11-github-copilot-canvas-adapter-poc/](sprints/sprint11-github-copilot-canvas-adapter-poc/) で、Local Ingestion Monitor を GitHub Copilot app Canvas から参照する thin adapter を追加する。M1 では `/create-canvas` skill workflow、project-scoped scaffold-first 方針、`--sanitized-only` Canvas-safe posture、bounded sanitized action contracts、raw/PII 禁止境界を canonical specs に反映した。製品コード、`.github/extensions/*`、依存関係、runtime artifact は M1 で追加していない。M2 では project-scoped extension scaffold を実行し、Canvas id `otel-monitor`、`open()`（loopback-only 検証、`/health/ready` 健全性チェック、非健全時診断ページ）、`onClose()` を実装。2026-06-29 の Codex 環境では `extensions_manage` / `extensions_reload` / Canvas validation tools が利用可能となったため blocker 解消。build 0/0、test 549/550（ConfigCli 300 + LocalMonitor 249、1件 pre-existing flaky concurrency test）。M3（minimal actions）へ移行。 |
