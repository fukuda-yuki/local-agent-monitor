...
| Local Raw Receiver Monitor | 実装中（M1/M2 完了 / M3 以降未着手） | [docs/sprints/sprint8-local-raw-receiver-monitor/](sprints/sprint8-local-raw-receiver-monitor/) で ConfigCli から共有 `Telemetry` / `Persistence.Sqlite` project を抽出した（M1）。M2 で `CopilotAgentObservability.LocalMonitor` ASP.NET Core host、loopback bind、Host header 検証、`POST /v1/traces` JSON/protobuf 受信、SQLite raw store 永続化、`413` body-size 境界、`--enable-raw-view` option（route は未登録）、および `profile-vscode-env --target monitor` / `--endpoint` を追加した。M2 は内部の非 shippable subset であり、bounded-channel queue/worker、`/health/*` readiness、sanitized projection + opt-in raw/PII route、Web UI/SSE、live validation は M3〜M6 で扱う。 |
