...
| Area | 状態 | 概要 |
| --- | --- | --- |
| Monitor Visual Implementation | **計画中 / Sprint13 candidate** | Sprint12 の [Design Shape](sprints/sprint12-monitor-design-shape/) を実装へ反映する後続タスク。`DESIGN.md`、`docs/sprints/sprint12-monitor-design-shape/milestones/M3-css-tokens/architecture.md`、`docs/sprints/sprint12-monitor-design-shape/milestones/M4-page-layouts/spec.md` を入力に、Local Monitor の `monitor.css`、共通レイアウト、Index / Traces / TraceDetail / Ingestions / Diagnostics の Razor 表示を整える。既存の telemetry 入力、SQLite schema、API field、raw / sanitized 境界、Canvas adapter contract は変更しない。検証は build、Playwright Chromium bootstrap、full test に加え、Local Monitor の主要ページ screenshot / browser smoke でレイアウト崩れ・raw 境界・`--sanitized-only` 表示を確認する。 |
| Monitor Design Shape | **完了** | [docs/sprints/sprint12-monitor-design-shape/](sprints/sprint12-monitor-design-shape/) で、Sprint10/Sprint11 後の Local Monitor visual foundation を docs-only の Shape / documentation alignment sprint として整理した。Sprint10 の実装前工程ではなく、既存の VS Code Dark+ テーマ方針、Noto Sans JP / Noto Sans Mono vendored typography、OKLCH color tokens、8px spacing、CSS token architecture、Index / Traces / TraceDetail / Ingestions / Diagnostics のページレイアウト仕様を履歴として再位置付けした。コード実装、telemetry 入力、schema、API field、raw 境界の変更はない。 |
