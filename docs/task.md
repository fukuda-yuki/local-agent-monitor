...
| Area | 状態 | 概要 |
| --- | --- | --- |
| Local Monitor Copilot Raw Analysis | **実装中 / .NET SDK live validation 部分完了** | [docs/sprints/sprint13-local-monitor-copilot-raw-analysis/](sprints/sprint13-local-monitor-copilot-raw-analysis/) で、Local Monitor TraceDetail から raw analysis run を開始し、.NET GitHub Copilot SDK analysis service へ dispatch する local surface を追加した。raw analysis result は local runtime data、GitHub 向けは repository-safe summary のみ。`--sanitized-only` では raw analysis UI / routes を無効化する。自動テストは追加済み。2026-06-30 の live validation で loopback 起動、synthetic trace ingestion、TraceDetail、analysis run 作成、repository-safe summary の raw marker / PII 非混入を確認した。ただし .NET Copilot SDK session は `failed` terminal status となり、signed-in Copilot SDK model response は未確認。Evidence: [sprints/sprint13-local-monitor-copilot-raw-analysis/milestones/M6-validation-and-closeout/live-validation.md](sprints/sprint13-local-monitor-copilot-raw-analysis/milestones/M6-validation-and-closeout/live-validation.md)。 |
