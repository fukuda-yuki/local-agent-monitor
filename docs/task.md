...
| LocalMonitor Windows Startup Surface | **実装中** | Windows Task Scheduler の user-level logon task として LocalMonitor を起動する `scripts/local-monitor/` 運用面を追加する。Task は current user / least privilege / loopback-only / multiple instances ignore-new を既定とし、DB / logs / state は `%LOCALAPPDATA%\CopilotAgentObservability\LocalMonitor\` 配下に置く。client routing は既存 `profile-vscode-env --profile raw-local-receiver --target monitor` を使い、Task 登録 script は VS Code 設定を書き換えない。 |
