...
| Local Raw Receiver Monitor | 実装中（M1/M2/M3 完了 / M4 以降未着手） | [docs/sprints/sprint8-local-raw-receiver-monitor/](sprints/sprint8-local-raw-receiver-monitor/) で ConfigCli から共有 `Telemetry` / `Persistence.Sqlite` project を抽出した（M1）。M2 で `CopilotAgentObservability.LocalMonitor` ASP.NET Core host、loopback bind、Host header 検証、`POST /v1/traces` JSON/protobuf 受信、SQLite raw store 永続化、`413` body-size 境界、`--enable-raw-view` option（route は未登録）、および `profile-vscode-env --target monitor` / `--endpoint` を追加した。M3 で bounded-channel ingestion queue + 単一 SQLite writer worker（commit 後のみ `2xx`、queue full `503` / commit timeout `504` / shutdown `503` / DB busy `503` / 非 busy 失敗 `500`）、`schema_version` additive migration（空の `monitor_ingestions` / `monitor_traces` を allowlist 列で作成、raw/PII なし、`raw_records` 維持、WAL + busy_timeout 並行読取）、readiness 閾値オプション（`--ingestion-stall-threshold-seconds` 既定 10 / `--projection-lag-threshold-seconds` 既定 60 + env）、および最初の `/health/live`・`/health/ready`（M3 は projection worker 不在のため常に `503` / `not_ready` で `projection_worker_missing`、誤って `ready` を返さない）を追加した。検証は build 0 警告・0 エラー、test 371 passing（300 ConfigCli + 71 LocalMonitor、M2 baseline 322 超）。M3 はまだ shippable ではなく、projection 投入 / `/api/monitor/*` cursor API、Web UI/SSE、opt-in raw/PII route、DR6 negative security matrix、live validation は M4〜M6 で扱う。 |
