...
| Area | 状態 | 概要 |
| --- | --- | --- |
| Local Monitor Copilot Raw Analysis | **実装中 / .NET SDK live validation 待ち** | [docs/sprints/sprint13-local-monitor-copilot-raw-analysis/](sprints/sprint13-local-monitor-copilot-raw-analysis/) で、Local Monitor TraceDetail から raw analysis run を開始し、.NET GitHub Copilot SDK analysis service へ dispatch する local surface を追加した。raw analysis result は local runtime data、GitHub 向けは repository-safe summary のみ。`--sanitized-only` では raw analysis UI / routes を無効化する。自動テストは追加済み。現環境では `GitHub.Copilot.SDK` package restore が TLS/credential error で失敗したため、実 SDK invocation は SDK runtime / package restore が利用可能な user environment で live validation が必要。 |
