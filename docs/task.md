...
| Sprint3: Content-aware Trace Diagnosis and Auto-decision Foundation | 要件定義中 | [docs/sprints/sprint3-trace-diagnosis/](sprints/sprint3-trace-diagnosis/) で trace content を含む診断候補生成、改善候補生成、自動採用判断の基盤を定義する |
...
- Sprint3 は要件定義中であり、Sprint2 MVP と Sprint2.5 で除外した trace からの自動診断を、content-aware evidence、改善候補生成、自動採用判断まで拡張して扱う。
...
- Sprint3 では、trace から failure category / anti-pattern 候補を自動抽出し、改善候補と自動採用判断に接続する。
