...
| Area | 状態 | 概要 |
| --- | --- | --- |
| GitHub Copilot Canvas Adapter PoC | **計画中 / M2 blocked** | [docs/sprints/sprint11-github-copilot-canvas-adapter-poc/](sprints/sprint11-github-copilot-canvas-adapter-poc/) で、Local Ingestion Monitor を GitHub Copilot app Canvas から参照する thin adapter を追加する。M1 では `/create-canvas` skill workflow、project-scoped scaffold-first 方針、`--sanitized-only` Canvas-safe posture、bounded sanitized action contracts、raw/PII 禁止境界を canonical specs に反映した。製品コード、`.github/extensions/*`、依存関係、runtime artifact は M1 で追加していない。M2 は project-scoped extension scaffold と `open()` diagnostics の予定だったが、2026-06-29 時点の Codex 環境では `extensions_manage` / `extensions_reload` / Canvas validation tools が callable ではないため、現行 blocker rule に従って手書き skeleton を作らず blocked として記録した。 |
