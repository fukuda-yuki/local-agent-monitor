...
test("emitted workspace IIFE requires explicit approval, sends zero-byte apply, and gives rollback failures terminal precedence", async () => {
    ...
    const selected = { draft_id: id, proposal_id: "proposal-1", root_id: "root-1", selection_revision: 2, approval_digest: "digest-2", state: "selected", files: [{ relative_path: "safe.txt", base_sha256: "base" }], hunks: [{ hunk_id: "hunk-a", relative_path: "safe.txt", selected: true }] };
...
test("emitted workspace IIFE fail-closes malformed, stale, and failed selected-draft reads", async () => {
    ...
    for (const selectedDraft of [response({ error: "draft_not_found", path: "secret", diff: "secret" }, 404), response({ draft_id: id, proposal_id: "proposal-1", root_id: "root-1", selection_revision: 2, approval_digest: "digest-2", state: "selected", files: [{}], hunks: [{ hunk_id: "keep", selected: true }], path: "secret" }), response({ draft_id: id, selection_revision: 3, approval_digest: "wrong", diff: "secret" })]) {
