...
test("emitted workspace IIFE fail-closes malformed, stale, and failed selected-draft reads", async () => {
    ...
    for (const selectedDraft of [response({ error: "draft_not_found", path: "secret", diff: "secret" }, 404), response({ draft_id: id, diff: "secret", path: "secret" }), response({ draft_id: id, selection_revision: 3, approval_digest: "wrong", diff: "secret" })]) {
