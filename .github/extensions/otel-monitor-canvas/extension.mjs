...
async function handleProposalApplyProxy(req, res, { monitorUrl, path }) {
    ...
    try {
        ...
        if (path.endsWith("/drafts") || /\/drafts\/[^/]+$/.test(path)) { const value = result.parsed; sendJson(res, result.response.status, { draft_id: value?.draft_id, proposal_id: value?.proposal_id, root_id: value?.root_id, selection_revision: value?.selection_revision, approval_digest: value?.approval_digest, state: value?.state, full_diff: value?.full_diff, diff: value?.diff, files: Array.isArray(value?.files) ? value.files.map(file => ({ relative_path: file.relative_path, base_sha256: file.base_sha256, replacement_sha256: file.replacement_sha256 })) : [], hunks: Array.isArray(value?.hunks) ? value.hunks.map(hunk => ({ hunk_id: hunk.hunk_id, relative_path: hunk.relative_path, selected: hunk.selected })) : [] }); return; }
...
const comparisonCandidateReasons = new Set(["not_exact_bound", ...comparisonReasons]);
const comparisonVerdicts = new Set(["improved", "no_change", "regressed", "insufficient_evidence"]);
const comparisonVerdictReasons = new Set(["invalid_linkage", "insufficient_cohort", "missing_quality_evidence", "post_severe_failure", "quality_improved", "quality_regressed", "missing_efficiency_evidence", "duration_improved", "duration_regressed", "tokens_improved", "tokens_regressed"]);
...
function comparisonResponse(value) {
    ...
    if (value.summary && typeof value.summary === "object") { const summary = pick(value.summary, ["verdict", "pre_pass", "pre_count", "post_pass", "post_count", "pre_duration_median", "post_duration_median", "duration_delta", "pre_token_median", "post_token_median", "token_delta"]); if (Array.isArray(value.summary.reasons)) summary.reasons = value.summary.reasons.filter(reason => typeof reason === "string" && comparisonVerdictReasons.has(reason)); safe.summary = summary; }
    if (Array.isArray(value.items)) safe.items = value.items.map(item => { const result = pick(item, ["objective_evaluation_id", "session_id", "run_id", "trace_id", "result", "severity", "evaluator_id", "evaluator_version", "criterion_id", "case_key", "recorded_at", "apply_id", "draft_id", "proposal_id", "proposal_revision", "selection_revision", "applied_at", "file_count", "state", "current_state", "status", "completeness", "started_at", "ended_at", "exact_bound", "evidence_available", "boundary_eligibility"]); if (Array.isArray(item.evidence_refs)) result.evidence_refs = item.evidence_refs.map(ref => pick(ref, ["kind", "reference_id"])); if (Array.isArray(item.suggestion_reasons)) result.suggestion_reasons = item.suggestion_reasons.filter(reason => typeof reason === "string" && comparisonCandidateReasons.has(reason)); return result; });
