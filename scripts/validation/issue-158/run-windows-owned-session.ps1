...
try {
 ...
 function Get-BlockerCode {
  ...
  if($value.schema_version-cne'issue-158-windows-blocker.v5'-or$value.candidate_sha-cne$CandidateSha-or$value.terminal_status-cnotin@('failed','canceled','timed_out','succeeded')){throw 'blocker'}
  ...
  $evidenceFailures=@('none','observer_missing','observer_multiple','source_application_version','protocol_version','client_start_count','status_observation_count','probe_session_count','execution_session_count','retained_root_count','retained_skill_count','probe_inventory_count','execution_inventory_count','prepared_invocation_count','same_client','exact_tool_union','retained_only_inventory','probe_native_reproof','execution_native_reproof','callback_native_reproof');if($value.execution_evidence_failure-cnotin$evidenceFailures){throw 'blocker'}
