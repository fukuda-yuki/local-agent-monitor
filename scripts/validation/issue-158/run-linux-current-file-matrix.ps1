...
try {
 ...
 function Get-LinuxChildDisposition([int]$ExitCode,$Output){
  if($ExitCode-ne1-or$Output.StderrNonempty){return 'internal'};try{$stdout=Get-LinuxChildText $Output 128}catch{return 'internal'};$codes=@('argument','source','top','prerequisite_dotnet','prerequisite_tool','clone','checkout','filesystem','test','test_timeout','test_killed','test_work_setup','test_native_preflight','test_native_matrix','test_route_setup','test_route_seed','test_host_start','test_metadata_route','test_historical_route','test_current_file_route','test_shutdown_drain','test_result_serialization','test_result_write','result','cleanup');$lines=@($stdout-split"`n")
