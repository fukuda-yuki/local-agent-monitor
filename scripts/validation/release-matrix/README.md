...
```powershell
pwsh -NoProfile -File scripts/validation/release-matrix/test-scan-outputs.ps1
```
...
```powershell
pwsh -NoProfile -File scripts/validation/release-matrix/run-automated-matrix.ps1
```

Scan a required repository-safe artifact:

```powershell
pwsh -NoProfile -File scripts/validation/release-matrix/scan-outputs.ps1 `
